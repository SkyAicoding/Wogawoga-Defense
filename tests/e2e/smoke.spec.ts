 ...
 * 배치/웨이브 빨리감기, 콘솔 에러 0 + 드로우콜 예산 어서션.
 *
 * ⚠ 이 파일에는 **성격이 다른 드로우콜 예산이 둘** 있다. 섞어 쓰면 둘 다 죽는다:
 *   · 아래 첫 테스트 — **가장 가벼운 전투 프레임**(타워 1기·웨이브1). 실측 17콜.
 *   · '기준 프레임 예산' 테스트 — **합성 기준 프레임**(타워 12기·적 60·아군 6). 예산 90콜.
 *     ⚠ 한때 '최악'이라 불렀다. 실측 파괴점은 **타워 17~19기**다 — 아래 레시피 머리말 참조.
 *   가벼운 프레임에 90을 대면 아무것도 안 잡히고, 최악 프레임에 17을 대면 언제나 빨갛다.
 */
...
/**
 * **합성 기준 프레임 레시피** — 아래 두 예산 테스트와 파괴점 스윕이 공유한다.
 *
 * ⚠⚠ **이 파일이 오랫동안 '최악 프레임'이라 부르던 것은 최악이 아니었다.**
 *   기본값 12기는 판당 건설 가능 빈칸(84~101개)의 약 **8분의 1**이고, 그나마도
 *   ① `continue outer` 가 z 루프를 넘겨 행당 1기만 서고 ② 신규 프로필 덱이 3종뿐이라
 *   여덟 중 다섯 종은 한 기도 안 섰다. 지금은 셋 다 고쳤다(매개변수 · 라벨 · 합성 카드).
 *
 *   '파괴점 스윕'(WGD_SWEEP=1) 실측 — 30표본 전부 그림자 true · 강등 0:
 *     타워 12 → s1 91% · s3 90% · s6 88%   |  타워 16 → 96% · 96% · 94%
 *     타워 18 → s1 **101%** ✗ · s3 99% · s6 97%
 *     타워 20 → s1 105% ✗ · s3 **103%** ✗ · s6 **101%** ✗
 *     타워 40 → 139% · 136% · 134%
 *   ⇒ **파괴점은 타워 17~19기**, 기울기는 타워당 약 2,400 삼각형.
 *   드로우콜은 12~40기 내내 43~48 로 평평하다(상한 56) — BatchedMesh 가 그 축은
 *   구조적으로 해결했다. **지금 걸리는 축은 삼각형 하나뿐이다.**
 *   삼각형은 타워 **개수**에 붙지 **종류**에 안 붙는다(12기에서 3종 135,172 대
 *   8종 134,252) — 타워를 몇 종 더 만드는 것은 이 축을 안 움직인다.
 *
 *   아래 두 예산 테스트는 그래서 **최악이 아니라 고정 기준점**을 잰다. 회귀 감시로는
 *   그대로 유효하다 — 같은 구성에서 값이 움직이면 무언가 바뀐 것이다.
 ...
 *
 * ⚠ **배치 루프는 한 판에 `gridH` 기가 상한이다** — 아래 `continue outer` 가 성공한 순간
 *   x 루프가 아니라 **z 루프**를 넘기므로 한 행에 최대 1기만 선다. 실측: 이 루프에 17·18·
 *   20·24기를 시켜도 s3(gridH 16)에서는 **정확히 16기**가 섰다. 여기서 세우는 12기는 여섯
 *   판 모두 gridH(15~16)보다 작아 레시피는 성립하고, 행마다 흩어지는 쪽이 한곳에 뭉치는
 *   것보다 컬링이 덜 먹어 **최악 프레임으로도 더 나쁘다**. 그래서 값은 고치지 않았다 —
 *   다만 12를 넘기려는 사람은 이 줄을 먼저 봐야 한다(호출부의 `>= 10` 어서션이 마지막
 *   방벽이다). 12기를 넘겨 재려면 `outer:` 를 x 루프에 붙이고 `continue outer` 를 써라.
 */
async function buildWorstFrame(
  ...
  const built = await page.evaluate((target: number) => {
    ...
    for (let k = 0; k < 3000 && st.enemies.length < 60; k++) {
      ...
      st.baseHp = 1e9;
      /*
       * ⚠ **한 웨이브의 편성에 기대지 않는다 — 같은 웨이브를 겹쳐 부른다.**
       *
       * 원래는 웨이브 49 하나만 불러 놓고 그것이 60마리를 채워 주기를 기대했다. 그런데
       * 그 수는 **웨이브 생성기의 편성에 딸린 값**이라 데이터가 바뀌면 조용히 무너진다.
       * 실제로 무너졌다: 2라운드에서 s3 에 공중 마릿수 상한이 들어가자 동시 생존이
       * 40 밑(28)으로 떨어져 `built.enemies >= 40` 가 빨개졌다 — **정작 재려던 예산은
       * 삼각형 96,470 / 드로우콜 36 으로 상한의 64% · 64% 였다.** 어서션이 아니라
       * 공허성 가드가 대신 운 것이다.
       *
       * 더 나쁜 것은 **옛 40마리가 무엇 덕이었는가**이다: `airLeft` 가 Infinity 라
       * 한 그룹이 `GROUP_MAX_COUNT`(25)까지 가던 버그로 s3 후반이 익룡 25마리 웨이브였고,
       * 그 스파이크가 정원을 채워 주고 있었다. 곧 옛 "최악 프레임"은 **버그 덕에 최악**이었다.
       * 버그를 고치면 최악도 같이 사라지는 잣대는 최악을 재는 잣대가 아니다.
       *
       * 그래서 편성에 기대지 않는다 — `waveIndex` 를 되돌려 **같은 웨이브를 목표에 닿을
       * 때까지 겹쳐 부른다**. 적은 위에서 불사·정지로 붙잡혀 있으므로 웨이브가 끝나지
       * 않고(= phase 가 prep 로 안 돌아온다) `callWave` 가 막히므로, phase 를 직접
       * 되돌린다 — 이 레시피는 이미 baseHp·maxHp·defId 를 직접 쓰는 합성 프레임이라
       * 같은 층위의 조작이다. 스폰이 끝날 시간을 주려고 300틱마다 한 번만 겹친다.
       */
      if (k % 300 === 299 && st.enemies.length < 60) {
        st.waveIndex = 48;
        st.phase = 'prep';
        g.callWave();
      }
...
test('타이틀 → 로비 → 전투 → 웨이브 진행 (콘솔 에러 0, 드로우콜 예산)', async ({ page }) => {
  ...
  /*
   * ── 문턱을 다시 유도한 자리 (이전: `<= 60`) ─────────────────────────────────
   * 60은 5단계 이전의 **최악 프레임 예산**이었다. 그런데 이 자리가 재는 프레임은
   * 최악이 아니라 **가장 가벼운 전투 프레임**이다 — 타워 1기·웨이브1·적 0~3마리.
   * 실측이 17콜이라 여유가 43이었고, 곧 어떤 회귀도 못 잡는 죽은 어서션이었다.
   * (진짜 최악 프레임은 아래 '최악 프레임 예산' 테스트가 직접 만들어 잰다 — 예산 90콜)
   *
   * 무엇을 재는 프레임인가: s1 · 타워 1기 · 웨이브1 진행 중 · 마을 Lv1.
   * 실측(swiftshader, 두 프로젝트 동일): **17콜** (40프레임 관측 15~17).
   *   유도: 적·타워 없는 전투 정지 프레임 11 + 타워 1기 3 + 적 1~2종 + 오버레이 ≈ 17
   * 문턱 24 = 실측 17 + 여유 7. 실측에 딱 붙이지 않는 이유는 웨이브1의 적 편성이
   * 밸런스에 따라 종을 한둘 더 낼 수 있어서다(종당 +1콜).
   * 이 문턱이 실제로 잡는 것: 비전투 씬에서 새던 **보스 예열 4메시**(+8콜)가 전투
   * 프레임으로 새어 들어오는 회귀 → 25콜 (render/views/enemyview.ts EnemyViewOptions).
   *
   * ⚠ **`drawCalls()` 한 번 읽기로는 못 잰다** — 이 자리에서 실제로 당했다.
   *   `drawCalls()`는 "마지막으로 그려진 프레임"을 돌려주는데, swiftshader 는 전투 첫
   *   프레임에서 씬 전체의 셰이더를 컴파일하느라 그 한 프레임이 500ms 를 넘긴다. 그래서
   *   `waitForTimeout(500)` 뒤에 읽으면 **아직 첫 프레임**이고, 그 첫 프레임이 하필
   *   보스 예열 프레임(17+8=**25콜**)이다. 빠른 기계에서는 17, 느린 기계에서는 25가
   *   나오는 계기였다(둘 다 실측). 예열 프레임은 전투 시작에 **한 번만** 나오는 의도된
   *   프레임이지 예산 프레임이 아니므로, 이 파일의 표준 잣대인 `maxFrame`(rAF 2회를
   *   건너뛰고 n프레임 관측)으로 **정상 프레임**을 잰다. 예열이 매 프레임으로 새면
   *   그때는 정상 프레임이 25가 되어 이 어서션이 잡는다 — 그게 이 문턱의 일이다.
   */
  await page.waitForTimeout(500);
  const light = await maxFrame(page, 20);
  expect(light.calls).toBeGreaterThan(0);
  expect(light.calls, `가벼운 전투 프레임 ${light.calls}콜 (실측 17)`).toBeLessThanOrEqual(24);
...
test('채집: 영구 그림 값은 +1 (버스트와 갈라 잰다) · 전 구간 예산 안', async ({ page }) => {
  ...
  /*
   * ⚠ 이 90은 **제품 예산**(README)이지 이 프레임의 회귀 문턱이 아니다 — 실측 15콜 /
   * 20,175삼각형이라 여유가 75다. 이 테스트가 재는 진짜 주장은 아래 **정착 Δ ≤ 1** 이고,
   * 예산에 대한 회귀 문턱은 '최악 프레임 예산' 테스트가 56으로 따로 맡는다.
   * (여기를 15 언저리로 조이면 웨이브 편성이 바뀔 때마다 무관하게 빨개진다)
   */
...
test('채집: 한 사이클이 실제로 돈다 · 사람 없으면 0 · 캔 자리는 비고 다시 자란다', async ({ page }) => {
  ...
  const regrow = await page.evaluate((taken) => {
    ...
    /*
     * ⚠⚠ **증인은 `regrowAt` 이 가장 이른 칸이어야 한다 — 셀 키 순서가 아니다.**
     *
     * 종전에는 `wait[0]`(= 셀 키 최솟값)을 증인으로 썼고, 그것이 이 항목을 **재생이
     * 죽어서가 아닌 이유로** 빨갛게 만들었다. 기제는 이렇다:
     *  · `updateRegrow` 는 (ㄱ) 재고가 문턱 위면 **한 칸도 안 되살리고 즉시 반환**하고,
     *    (ㄴ) 아래면 **`regrowAt` 이 이른 칸부터** 재고가 문턱에 닿을 때까지만 되살린다.
     *  · 그런데 셀 키 최솟값 칸은 자격을 **가장 늦게** 얻을 수도 있다. 그러면 밭을 비운
     *    직후의 단 한 번뿐인 재생 펄스를 더 이른 칸들이 먼저 먹어치우고, 재고가 문턱에
     *    닿아 (ㄱ)이 닫힌다 — 증인 차례는 영영 안 온다.
     *
     * 실측(스테이지1 · 배포본 문턱 0.35 · 같은 순서):
     * ```
     *   최소 지연 9000 → 비운 시점의 자격자 **0개** → 증인이 자란다 (잔여 재고 2.85%)
     *   최소 지연 3000 → 비운 시점의 자격자 **9~15개** → 그들이 문턱까지 채우고 닫힌다
     *                     증인은 918틱 뒤에 자격을 얻는데 그땐 이미 문턱 위 (잔여 28.1%)
     * ```
     * 곧 재생은 **죽은 게 아니라 더 잘 돌았고**, 하필 증인이 안 뽑혔을 뿐이다.
     * 이 파일의 위 주석이 정확히 이 실패를 예고해 뒀다("지연이나 일꾼 수를 밸런싱하는
     * 순간 그 우연은 사라지는데, 그때 이 항목은 재생이 죽어서가 아니라 밭이 안 비어서
     * 빨개진다 — 그건 무엇이 틀렸는지 말해 주지 못하는 실패다").
     *
     * 그래서 **문턱을 낮추는 게 아니라 증인을 `updateRegrow` 와 같은 규약으로 고른다**:
     * `(regrowAt 오름차순, 셀 키 오름차순)`. 그 칸은 재고가 문턱 아래인 한 **반드시
     * 첫 번째로 뽑히는 칸**이라, 계약("밭이 문턱 아래면 자격 있는 칸이 자란다")을 한 톨도
     * 약화시키지 않고 결정론적으로 성립한다.
     */
    const key = (r: { cellX: number; cellZ: number }): number => r.cellZ * 1000 + r.cellX;
    const queue = wait.slice().sort((a, b) => a.regrowAt - b.regrowAt || key(a) - key(b));
    const target = queue[0]!;
    /*
     * 다른 칸 하나에는 타워를 세워 재생권을 태운다 (R3).
     * ⚠ **줄에서 두 번째로 이른 칸**을 고른다 — 태우지 않았다면 증인 바로 다음에
     *   반드시 되살아났을 칸이라, "안 자랐다"가 우연이 아니라 **소각 때문**임이 선다.
     *   아무 칸이나 고르면 애초에 뽑힐 차례가 아니어서 판별력이 없다.
     */
    const burn = queue.length > 1 ? queue[1]! : null;
    let burnBuilt = false;
    if (burn) {
      g.setGold(99_999);
 ...
 * (그 자리는 이제 자기가 재는 프레임 — 가장 가벼운 전투 프레임 — 에서 다시 유도했다:
 *  실측 17콜 → 문턱 24. 이 테스트가 최악 프레임을 맡고, 그쪽이 가벼운 프레임을 맡는다.)
 * 실제로 재 보면 그 전제는 틀렸다:
 *
 ...
 *
 * ── 타워 인스턴싱 뒤 재측정: **천장을 만들던 것이 사라졌다** ────────────────
 * 위 문단들의 "타워 1기당 약 3콜 · 상한 없음" 과 콜 수는 타워가 개별 Mesh 이던 시절
 * 값이다(백분율도 그때의 예산 90 기준이다). 지금은 전 타워가 `BatchedMesh` 묶음
 * 여섯으로 그려진다 — render/views/towerbatch.ts.
 *
 *   같은 레시피(타워 12기 · 적 56 · 아군 6)   desktop 76콜 → **42콜 / 128,343**
 *                                              mobile  → **43콜 / 129,385**
 *   타워 수 스윕(실제 렌더러 · 8종×티어 섞기 · 적 0)
 *     전  0기 10콜 · 4기 22 · 8기 34 · 12기 46 · 24기 82 · 40기 130   (= 10 + 3n)
 *     후  0기 10콜 · 4기 14 · 8기 14 · 12기 14 · 24기 14 · 40기 14   (상수)
 *   삼각형은 두 표가 **한 자리도 다르지 않다**(묶기가 정점을 늘리지 않는다).
 *
 * 그래서 이 파일의 최악 프레임 문턱을 90 → **56** 으로 다시 유도했다(아래 주석).
 * 삼각형 예산 150,000은 그대로다 — 그쪽은 타워가 만든 천장이 아니었다.
 */
test('기준 프레임 예산(타워 12기) — 삼각형 150,000 / 드로우콜 상한', async ({ page }) => {
  /*
  ...
  /**
   * 드로우콜 상한 **56** — 타워를 묶은 뒤 다시 유도한 값이다.
   *
   * 옛 문턱 90은 "실측 73~81 + 타워 세 기어치 여유"였는데, 그 73~81을 만들던 타워 몫이
   * **기당 3콜에서 0으로** 바뀌었다(위 재측정 표). 90을 그대로 두면 타워를 다시 개별
   * Mesh 로 되돌려도 12기까지는 아무도 모른다 — 여유가 곧 눈가리개가 된다.
   *
   * 유도: 실측 최댓값 43(6판 × 2뷰포트 · 41~43) + 반복 흔들림 2 + **새 메시 열한
   * 가지어치**(적 종을 하나 더하면 인스턴스 메시 +1콜, 그림자 캐스터면 +2) = 56.
   * 이 값을 올려야 할 만큼 메시가 늘었다면 그건 **구조를 고칠 신호**다.
   */
  expect(worst.calls, msg).toBeLessThanOrEqual(56);
...
/**
 * **스테이지별 기준 프레임 예산(타워 12기)** — 위 테스트가 s1만 재던 구멍을 메운다.
 *
 * ⚠ 이름에서 '최악'을 뺐다. 실측(아래 파괴점 스윕)이 이 구성은 최악이 아니라
 *   **고정 기준점**임을 보였기 때문이다 — 재는 것과 다른 이름을 붙이는 것이
 *   이 저장소가 세 번 당한 병이다. 회귀 감시로서는 그대로 유효하다:
 *   같은 구성에서 값이 움직이면 무언가 바뀐 것이다.
 *
 ...
 *
 * 실측 — **레시피가 웨이브를 겹쳐 부르고 전 종을 세우게 된 뒤**(desktop 1280×800 · swiftshader):
 *   s1 43콜 / 136,171   s2 41 / 135,884   s3 44 / 135,172
 *   s4 42 / 132,924     s5 43 / 134,185   s6 41 / 130,137
 * 여섯 판이 삼각형 예산의 86.8~90.8% 안에 모인다.
 *
 * ⚠⚠ **이것은 "최악"이 아니라 타워 12기짜리 기준 프레임이다.** 아래 '파괴점 스윕'이
 *   실측했다(30표본 전부 그림자 true · 강등 0):
 *     타워 12 → s1 91% · s3 90% · s6 88%
 *     타워 **18** → s1 **101%** ✗ · s3 99% · s6 97%
 *     타워 **20** → s1 105% ✗ · s3 **103%** ✗ · s6 **101%** ✗
 *     타워 40 → s1 139% · s3 136% · s6 134%
 *   곧 **파괴점은 타워 17~19기**이고 기울기는 타워당 약 2,400 삼각형이다.
 *   판당 건설 가능 빈칸이 84~101개이므로 잘 두는 사람은 이 선을 훌쩍 넘긴다.
 *   드로우콜은 12~40기 내내 43~48 로 평평하다(상한 56) — BatchedMesh 가 그 축은
 *   구조적으로 해결했다. **지금 걸리는 축은 삼각형 하나뿐이다.**
 *
 * ⚠ 삼각형은 타워 **개수**에 붙지 **종류**에 안 붙는다 — 12기에서 3종 135,172 대
 *   8종 134,252 로 사실상 같다(BatchedMesh 가 지오메트리를 공유한다).
 *   곧 타워를 몇 종 더 만드는 것은 이 축을 안 움직인다.
 *
 * ⚠ **그 전 표(아래)와 나란히 읽으면 안 된다 — 레시피가 달랐다.**
 *   s1 43콜/129,777 · s2 41/120,942 · s3 43/136,340 · s4 42/132,368 · s5 43/131,597 · s6 41/131,015
 *   그때는 판마다 **웨이브 49 하나가 주는 만큼만** 세웠으므로 s2 가 120,942 로 낮은 것은
 *   s2 가 가벼워서가 아니라 그 판의 그 웨이브가 적게 준 것이다 — 판끼리 비교가 안 됐다.
 *   지금은 여섯 판 전부 적 60마리로 맞춰지므로 표가 처음으로 나란히 읽힌다.
 *   (더 옛날, 타워가 개별 Mesh 이던 시절의 콜은 73~76 이었다. 묶기가 32~33콜을 줄였고
 *    삼각형은 그때도 지금도 안 늘렸다 — 묶기는 정점을 늘리지도 줄이지도 않는다.)
 *
 * ⚠⚠ **이 표는 아직 "최악"이 아니다.** 레시피가 세우는 타워는 12기인데 판당 건설 가능
 *   빈칸은 84~101개다(약 8분의 1). 타워 수를 늘렸을 때 어디서 예산을 깨는지는 별도
 *   과제이고, 그때는 품질 자동 강등(app.ts:200 · quality.ts)이 측정 중에 끼어들어
 *   그림자 패스를 통째로 지울 수 있다는 것을 함께 다뤄야 한다 — 이 저장소가 실제로
 *   당한 사고이고 그 증인 코드(gl.viewport 후킹)는 지워졌다.
 * 한 판이 약 18초라 6판 + 로딩까지 **1.8분**이다
...
test('스테이지별 기준 프레임 예산(타워 12기) — s1~s6 전부 삼각형 150,000 / 드로우콜 56', async ({
  page,
}, testInfo) => {
...
test('하늘길: w22부터 익룡이 공중 레인으로 온다 (렌더 + 예산)', async ({ page }) => {
  ...
  // ⚠ 아래 90도 제품 예산이다 — 이 프레임 실측은 14콜 / 29,305삼각형(여유 76).
  //   여기서 재는 주장은 "익룡이 공중 레인으로 온다"이고 예산은 곁다리 확인이다.
