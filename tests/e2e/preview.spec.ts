...
test('도감 미리보기: 여닫아도 WebGL 컨텍스트가 안 샌다', async ({ page }) => {
  /*
   * ⚠ 기본 60초로는 **CI 에서만** 터진다. 이 계약은 WebGL 컨텍스트를 20번 만들었다
   *   버리는 것이 본체인데, GitHub 러너의 소프트웨어 GL 은 이 컨테이너보다 두 배 넘게
   *   느리다(같은 e2e 묶음: 로컬 5.5분 · CI 11.1분). 그래서 로컬은 초록인데 CI 만
   *   `Test timeout of 60000ms exceeded` 로 빨갰다 — **재는 대상이 아니라 시계가
   *   짧았던 것**이다. 시간을 늘려도 계약이 재는 것(`__lost === 0` · 컨텍스트 경고 0 ·
   *   20회)은 한 글자도 안 바뀐다. ROUNDS 를 줄이는 쪽으로 맞추면 상한(16)을 못 넘겨
   *   계약이 공허해지므로 그쪽으로는 가지 않는다.
   */
