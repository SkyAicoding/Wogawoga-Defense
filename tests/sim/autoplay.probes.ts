 ...
 * 안 움직인다. 곧 유도 기준은 ⟦원장 1a.clearRate = 77.50%⟧ 이고 +5%p 는 0.8250 이다.
 * 그런데 문턱을 그렇게 실측에 붙여 잡으면 **블록 둘(대조군 fast 등급)의 산포 안**에 들어와
 * 대조군 스위트가 통째로 판정 불가에 걸린다 — 계약이 아니라 잡음을 재게 된다는 뜻이다.
 * 그래서 §8-8 이 이름과 함께 적어 둔 **0.90** 을 그대로 쓴다: full 여유 **12.50%p** 이고,
 * 0.95 상한도 지킨다. (이 여유는 문간 두 라운드에 걸쳐 완주율이 80.63% → 78.75% →
 *  **77.50%** 로 내려오며 9.37%p → 11.25%p → **12.50%p** 로 **넓어졌다** — 문턱은 한 자리도
 *  안 움직였다. 곧 이 다리는 게임이 어려워질수록 안전해지는 방향이다)
 ...
 *    · 지금 표본의 분포가 ⟦원장 3.dist = [[5,1],[6,1],[7,7],[8,151]]⟧ 이다. 곧 옛 극값 계약
 *      `전 시드 ≥ 7` 을 그대로 되살리면 **배포본이 즉시 빨갛다**(5가 한 판 · 6이 한 판).
 *      옛 계약이 초록이었던 것은 게임이 그것을 만족해서가 아니라 **20시드가 그 판들을
 *      못 봤기 때문**이다 — 이 항목이 극값 금지 규칙의 근원이 된 바로 그 사실이다.
 *      ⚠ 문간 정지 자리 재유도로 분포가 [[6,3],[7,7],[8,150]] → 위 값으로 옮겼다. 사고 수는
 *        3판 → 2판으로 **줄었지만** 그중 하나가 **하드 바닥 5**에 앉았다(`3.floor` 여유 0).
 *        곧 이 항목의 결론("복원 불가")은 약해진 게 아니라 **강해졌다**.
 *    · 강도를 비교하면: 극값 계약이 n=20 에서 95% 확률로 통과하려면 사고율 q ≲ 0.26%
 *      여야 한다. 실측 q 는 2/160 = **1.25%** 로 그 **약 다섯 배**다. 곧 옛 계약이 암묵적으로
 *      주장하던 사고율은 배포본이 이미 만족하지 않는다.
 *    · 그러면 문턱을 2% → 1.25% 근처로 조이면 되는가? **안 된다.** 그건 실측을 보고
 ...
 *    · ⚠⚠ **그런데 지금 여유가 0.12%p(사고 3판 대 허용 3.2판)뿐이다.** 문간 라운드가
 *      사고를 1판 → 3판으로 올렸다(마을이 문 앞에서 실제로 쏘게 되면서 판이 더 오래
 *      가고, 오래 갈수록 타워가 더 부서진다). **한 판만 더 늘면 빨갛다.** 여기가 다음에
 *      걸리면 손댈 곳은 문턱이 아니라 습격대 화력이다 — 바로 아래 문단이 이미 그렇게
 *      적어 두었고, 이제 그 문장이 가정이 아니라 임박한 일이 됐다.
 ...
 *     ④ ⇒ `updateRegrow` 의 재고 합이 매 틱 **분모와 같고**(아무 칸도 안 텄으므로 재고 1.0),
 *        문턱은 1 이하라 그 비교 한 줄에서 반환한다 →
 *        **상태 변화 0 · 이벤트 0 · rng 소비 0** (economy.fillHand 의 드로우가 안 밀린다)
 *        ⚠ 옛 문장은 "첫 줄 `regrowAt === 0` 이 전 칸에서 참"이었다 — 재생의 방아쇠가
 *          시간에서 **재고 비율**로 옮겨지면서(R7) 그 줄이 사라졌다. 결론은 그대로다.
 ...
 * 독립 4블록 합산은 같은 문턱 1.25 에서 ⟦원장 11b.blocked = 2.5656⟧ = 여유 0.446 이고,
 * 인접 합산은 1.264 = 여유 0.014 였다. 곧 **문턱을 한 자리도 안 건드리고 여유의 자릿수가
 * 달라진다**. 이 항목은 표본만 고쳐도 크게 안전해진다.
 *
 * ⚠⚠ **한때 이 항목의 여유는 통째로 꼬리가 만든 것이었다 — 그리고 그것이 고쳐진 자리다.**
 *   문간 라운드 전에는 판별로 짝지은 봉쇄비의 **중앙값이 1을 밑돌았고**(0.967) 비가 1을
 *   넘는 시드도 절반이 안 됐다(43.75%). 곧 "판의 과반에서는 정원 6이 정원 2보다 덜
 *   붙잡는다"였고, 합산 비 1.47 을 만드는 것은 소수의 위급한 판이었다. 기전은 아군의
 *   값이 아니라 **집결 대열의 기하**였다 — 여섯이 한 점에 서서 같은 적을 보고 있었다.
 *   `ALLY_MUSTER_ROW_GAP`(대열이 마을 **앞으로** 깊어진다) 재유도 뒤 실측이 뒤집혔다:
 *     ⟦원장 11b.pairedMedian = 중앙값 2.100⟧ · ⟦원장 11b.pairedMedian = 비>1 시드 78.62%⟧
 *   곧 **판의 과반에서 정원 6이 정원 2보다 더 붙잡는다.**
 *   ⚠ **가동률의 부호도 이번(문간 정지 자리 재유도) 라운드에서 뒤집혔다** — 한때 이 자리에
 *   "가동률은 여전히 정원 6쪽이 낮다"고 적혀 있었는데 실측은
 *   ⟦원장 11b.pairedMedian = 가동률 0.40% → 0.41%⟧ 로 **정원 6쪽이 오히려 높다**.
 *   곧 "늘어난 인원이 놀고 있다"는 옛 진단은 더 이상 참이 아니다. 두 값이 0.4% 대에서
 *   거의 붙어 있으므로 크기 주장을 걸지 않고 **보고 대상으로만 남긴다** — 이 문단이
 *   아래에서 스스로 경고하는 "부호가 뒤집힌 기록"의 재발을 막는 것이 이 줄의 몫이다.
 ...
 * 실측은 ⟦원장 12.frontOnly = 3.75%⟧ 다(한때 이 자리에 "3/80 = 3.75%"라고 적혀 있었는데
 * 같은 실행이 쓴 원장과 어긋나 있었다 — 낡은 기록이라 원장 인용으로 바꿨다).
 * ⚠ **한때 이 자리의 여유가 0 이었다 — 지금은 아니다.** 문간 첫 라운드가 이 값을
 *   2.50% → **5.00%(4/80)** 로 밀어 올려 문턱 0.05 를 `≤` 하나로 통과하던 시기가 있었다.
 *   문간 정지 자리 재유도(적이 제 메시 길이만큼 물러나 선다)가 이를 **3.75%(3/80)** 로
 *   되돌려 여유가 **한 판**으로 회복됐다. 곧 이 항목은 더 이상 봉투에서 가장 얇은 자리가
 *   아니다 — 그 자리는 **`3.floor`(실측 5 · 문턱 ≥5, 여유 0)** 로 옮겨 갔다.
 *   여기서 다시 걸리면 문턱이 아니라 **입구와 마을 앞의 화력 차**를
 *   다시 유도해야 한다(= `12.slackSign` 이 재는 축과 같은 것을 승률 눈금으로 본 것이다).
 ...
 *     빨갛다"고 적었는데, **같은 실행이 쓴 원장은 ⟦원장 12.rateGap = 27.50%p⟧** 다
 *     (⟦원장 12.home.dominates = 승 67/80 대 45/80⟧). 곧 배포본은 옛 문턱을 1.25%p 여유로
 *     통과하고 있었다(문간 라운드 전에는 23.75%p 였다 — 여유가 8.75%p 에서 1.25%p 로
 *     줄었고, 그것이 아래 (2)의 "승률 축으로는 이 크기를 못 진다"를 더 강하게 만든다). 인용한 "320시드 합산 15.000%p" 는 창 `stance` 에서 나온 값이 아니고,
