 ...
 * 안 움직인다. 곧 유도 기준은 ⟦원장 1a.clearRate = 78.75%⟧ 이고 +5%p 는 0.8375 이다.
 * 그런데 문턱을 그렇게 실측에 붙여 잡으면 **블록 둘(대조군 fast 등급)의 산포 안**에 들어와
 * 대조군 스위트가 통째로 판정 불가에 걸린다 — 계약이 아니라 잡음을 재게 된다는 뜻이다.
 * 그래서 §8-8 이 이름과 함께 적어 둔 **0.90** 을 그대로 쓴다: full 여유 **11.25%p** 이고,
 * 0.95 상한도 지킨다. (이 여유는 문간 라운드에서 완주율이 80.63% → 78.75% 로 내려오며
 *  9.37%p → 11.25%p 로 **넓어졌다** — 문턱은 한 자리도 안 움직였다)
 ...
 *    · 지금 표본의 분포가 ⟦원장 3.dist = [[6,3],[7,7],[8,150]]⟧ 이다. 곧 옛 극값 계약
 *      `전 시드 ≥ 7` 을 그대로 되살리면 **배포본이 즉시 빨갛다**(6이 세 판 있다).
 ...
 *      여야 한다. 실측 q 는 3/160 = **1.88%** 로 그 **일곱 배**다. 곧 옛 계약이 암묵적으로
 *      주장하던 사고율은 배포본이 이미 만족하지 않는다.
 *    · 그러면 문턱을 2% → 1.88% 근처로 조이면 되는가? **안 된다.** 그건 실측을 보고
 ...
 *  · 문턱 0.30 의 유도: 배포본 실측이 창 `strong` 에서 ⟦원장 1b.tailCvar = 38.75%⟧ 이고,
 ...
 * 독립 4블록 합산은 같은 문턱 1.25 에서 ⟦원장 11b.blocked = 1.6959⟧ = 여유 0.446 이고,
 ...
 *     ⟦원장 11b.pairedMedian = 중앙값 1.111⟧ · ⟦원장 11b.pairedMedian = 비>1 시드 56.25%⟧
 *   곧 **판의 과반에서 정원 6이 정원 2보다 더 붙잡는다.** 다만 가동률은 여전히 정원 6쪽이
 *   낮다(⟦원장 11b.pairedMedian = 가동률 1.13% → 0.73%⟧) — 한계 인원이 노는 시간이
 *   완전히 사라진 것은 아니고, 그래서 이 줄은 **보고 대상으로 남긴다**.
 ...
 * 실측은 ⟦원장 12.frontOnly = 5.00%⟧ 다(한때 이 자리에 "3/80 = 3.75%"라고 적혀 있었는데
 * 같은 실행이 쓴 원장과 어긋나 있었다 — 낡은 기록이라 원장 인용으로 바꿨다).
 * ⚠⚠ **여유가 0 이다.** 문턱이 0.05 이고 실측이 정확히 5.00%(4/80)라 `≤` 하나로 통과한다 —
 *   한 판만 더 front-only 로 넘어가면 빨갛다. 문간 라운드가 이 값을 2.50% → 5.00% 로
 *   올렸고(마을이 문 앞에서 실제로 쏘면서 두 국면의 차이가 커졌다), 그래서 이 자리는
 *   이제 이 항목에서 가장 얇다. 여기서 걸리면 문턱이 아니라 **입구와 마을 앞의 화력 차**를
 ...
 *     빨갛다"고 적었는데, **같은 실행이 쓴 원장은 ⟦원장 12.rateGap = 16.25%p⟧** 다
 *     (⟦원장 12.home.dominates = 승 58/80 대 45/80⟧). 곧 배포본은 옛 문턱을 1.25%p 여유로
