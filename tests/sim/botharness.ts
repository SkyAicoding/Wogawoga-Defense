 ...
 *     ⚠ **9단계(정지 사격 개편)로 이 항목의 근거가 낡았다.** 습격대 전원이 원거리
 *       2.4~3.6이 되어 SAFE_DIST(2.0)는 더 이상 안전하지 않다. 값을 그대로 두는
 *       이유와 고치는 일의 범위는 SAFE_DIST 선언부 주석에 적었다.
 *
 ...
 * 봇이 목표로 삼는 경로 이격 거리.
 *
 * ⚠ **이 값의 유도 근거는 9단계(정지 사격 개편)로 낡았다 — 값은 일부러 그대로 둔다.**
 * 옛 근거: "근접 습격대가 절대 닿지 못하는 거리. lancer 사거리 1.95가 최대치이고
 * 타워 좌표는 셀 정수이므로 2.0이면 확정 안전이다(archer 3.2 / hexer 3.6은 여전히
 * 닿는다 — 원거리는 '거리로 푸는' 위협이 아니다)."
 * 지금은 전위도 2.4~2.8이라 **2.0은 안전하지 않다**. 새 확정 안전선은 4.0(hexer 3.6 밖)
 * 인데, 그 거리에서 경로를 덮는 타워는 ballista(5.5) 하나뿐이라 값을 4.0으로 올리면
 * 나머지 일곱 타워가 전부 "덮지 못함" 등급으로 떨어져 봇이 도로 경로에 밀착한다
 * (= 안전 봇과 밀착 봇이 같아져 봉투 4번이 공허해진다).
 *
 * 즉 이 상수를 고치는 일은 **봇의 배치 전략을 다시 설계하는 일**이고, 그러면 봉투의
 * 모든 기준선(승수·파괴·타워 수·갈래 비교)이 한꺼번에 움직인다. 시뮬레이션 단계에서
 * 할 일이 아니라 밸런스 단계의 첫 항목이다 — autoplay.test.ts 헤더의 적색 항목 1번 참조.
 */
