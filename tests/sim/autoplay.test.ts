...
/**
 * **판당 절대 누수 피해** — 기지가 실제로 맞은 양(`BotResult.leaked`).
 *
 * `slackAvg`와 달리 **마을 레벨에 완전히 중립**이다. 마을을 올린 갈래는 baseHpMax가
 * 커져 (H−D)/H가 저절로 올라가므로, "덜 맞았는가"를 여유로 물으면 **HP를 산 쪽이
 * 더 많이 맞고도 이기는** 오독이 생긴다(7번 항목 주석의 실측 21.3 대 20.7).
 * 갈래끼리 "누가 덜 맞았나"를 비교할 때는 반드시 이쪽을 쓴다.
 */
...
describe('autoplay 난이도 봉투', () => {
   ...
   *
   * ⚠ **Phase 4 정정 — 위 0.8023은 15단계 시점 값이고 지금은 0.7080이다.**
   ...
   *
   * ── 살점 값 단계: **위 ⚠가 예고한 자리에서 걸렸고, 예고된 대로 고쳤다** ──────
   * 살점 값(수입을 개체 처치가 아니라 몫 단위로 지급)을 넣자 기지(자연) 갈래가
   * `여유` 다리에서 걸렸다. 그런데 실측을 보면 **정확히 위 ⚠가 서술한 오독**이다:
   * ```
   *   타워 기준선  60/80 · 여유 18.50% · 누수피해 20.7/판
   *   기지(자연)   63/80 · 여유 20.74% · 누수피해 21.3/판   ← 여유는 높은데 피해는 더 받는다
   * ```
   * 곧 기지 갈래는 **더 많이 맞고도** 여유가 높다 — 마을이 baseHpMax를 키웠기 때문이다.
   * 이 상태를 '지배 전략'이라 부르면 지표가 거짓말을 하는 것이다.
   *
   * 그래서 두 번째 다리를 `slackAvg`(마을 HP에 오염된 양)에서 **판당 절대 누수 피해**
   * (`BotResult.leaked` — 마을 레벨에 완전히 중립이다)로 다시 유도했다.
   * **문턱을 낮춘 것이 아니다**: 첫 다리(승률)는 그대로이고, 두 번째 다리는 "덜 맞는가"라는
   * 원래 묻고 싶었던 질문을 처음으로 정직하게 묻는다. 오히려 마을 갈래가 여유를 사서
   * 통과하던 구멍이 막혔으므로 **판별력은 올라간다**.
   * 판별력 확인: 네 갈래 전부에서 `지배[누수피해]`가 false이고, 그중 기지(자연)만
   * 첫 다리(승률)를 넘는다 — 곧 이 다리가 실제로 판정을 가르는 유일한 자리다.
   */
  it('골드 배분 네 갈래 — 적정 배분은 살아 있고, 몰빵은 무너진다', () => {
    ...
    for (const [name, rs] of branches) {
      ...
      // 그리고 **더 많이 이기면서 덜 맞으면** 실패 (불도저와 같은 잣대).
      // 두 번째 다리가 여유가 아니라 **절대 누수 피해**인 이유는 위 살점 값 절 참조 —
      // 여유는 마을 갈래에서 baseHpMax에 오염돼 "더 맞고도 높은" 값이 나온다.
      const dominant = rate(rs) > rate(tower) && leakDmg(rs) < leakDmg(tower);
      expect(dominant, `${name} 갈래가 더 많이 이기면서 덜 맞는다 = 지배 전략: ${msg}`).toBe(false);
    }
   ...
   * 위약은 그 시간을 통째로 놀린다. 승수 어서션(`진짜 ≥ 위약`)의 여유가 8단계의 0판에서
   * 18판으로 벌어졌다.
   */
   ...
   * ⚠ 잔여 HP는 부족 쪽이 앞서지만(238 대 199) 승수가 뒤지므로(29 대 33) 지배 전략 연언은
   *   서지 않는다. 그 잔여합은 마을 레벨업이 baseHpMax를 키운 몫이 대부분이다 —
   *   판당 비율로 정규화하면 19.19% 대 19.90%로 오히려 부족 쪽이 아래다
   *   (BotResult.baseHpMax 주석이 경고하는 바로 그 오독이라 여기 적어 둔다).
   */
  ...
  it('정원 곡선 — 마을 레벨마다 부족이 늘고, 실제로 더 많이 나가 더 많이 붙잡는다', () => {
    ...
    // (실측 160시드 합산 생산비 1.294 · 봉쇄비 1.285. 열이 평탄해지면 둘 다 1.000이 된다)
   ...
   *
   * ── 실측 (스테이지1 · 40시드 블록 4벌 = 160시드 · 경로 36.19타일) ───────────
   ...
   *
   * ⚠⚠ **다만 `부족 ≤ 타워` 한 줄은 지금 동전 던지기다 (미해결, 보고만 한다).**
   * 커밋된 12시드 블록에서 두 팔의 웨평이 **정확히 같다**(51.33 대 51.33 — 시드별 차가
   * +4/−1/−3/0/0/0/−1/0/−3/+4/−1/+1로 상쇄된다). 시작점을 옮기면 부호가 뒤집힌다:
