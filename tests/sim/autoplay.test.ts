 ...
 *     효과크기가 ⟦원장 6.dozer.notDominant = MDE 3판/80⟧ 이다 — 불도저 쪽으로 다섯 판만 더
 *     갈리면 빨개진다. 옛 α(0.05/12)에서는 같은 계산이 더 컸다 — 곧 α 교체의 실제 효과는
 *     "불가능 → 유한"이 아니라 **유한 → 더 작은 유한**이다. (한때 "산술적으로 실패가 불가능했다"고
 ...
 * ⟦원장 cal.slope = 완주율 80.00% → 93.75%⟧ · ⟦원장 cal.slope = 여유 21.40% → 48.85%⟧)을
 * 그 술어로 재니 **거짓**이었다 —
