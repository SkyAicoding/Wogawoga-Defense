 ...
 *  2) 습격대가 실제로 값을 청구한다 — 파괴 합계 하한 (실측 103기, 문턱 100 — 여유 3기)
...
describe('autoplay 난이도 봉투', () => {
   ...
   *   커밋 블록 실측: home **72.5%** 웨평 49.23 여유 11.50% 최전선 32.74 생존 3,600틱 ·
   *   front **57.5%** 웨평 48.95 여유 6.30% 최전선 0.00 생존 1,024틱(생존비 **3.52배**).
   *   곧 격차가 **정확히 15.0%p = 문턱 15%p**다 — 소수점까지 동률이라 한 판이면 빨개진다.
   *   문턱을 유도할 때 근거로 든 "실측 최악 27.5%p의 절반 남짓"이라는 문장은 지금 거짓이고,
   *   실제 격차는 그 유도값의 **절반**이다. 언제 좁아졌는지는 **불명**이다.
   *   ⚠ 배포본 25f2b57을 같은 하네스로 되돌려 재면 독립 블록 5000·9000에서 각 −7.5%p로
   *     **이미 위반**이고, 살점 값이 그 두 위반을 해소했다(+2.5 / +10.0 / +0.0%p).
   *     곧 이 항목은 살점 값이 **살려 놓은** 자리이지 깎은 자리가 아니다.
   *
