...
describe('autoplay 난이도 봉투', () => {
   ...
   *      주로 재기 때문인데(위 ⚠⚠), 정원 축에서는 그 걱정이 없다 — **양 팔이 같은
   *      국면을 겪으므로** 위급도가 공통이고 차이는 머릿수에서만 나온다.
   *
   *      실측(스테이지1 · 40시드 블록 4벌 = 160시드 · minNear 1 · 예비비 600. 생산/봉쇄):
   *        블록      1000        1001        1002        1003
   *        정원 2   522/22,356  572/31,115  539/21,999  615/29,389
   *        정원 6   692/31,750  749/46,386  655/32,096  748/44,936
   *        생산비    1.326       1.309       1.215       1.216   → 최악 **1.215**
   *        봉쇄비    1.420       1.491       1.459       1.529   → 최악 **1.420**
   *      문턱은 생산 1.10 · 봉쇄 1.25로 최악에서 각각 9%p·14%p 아래다.
  ...
  it('정원 곡선 — 마을 레벨마다 부족이 늘고, 실제로 더 많이 나가 더 많이 붙잡는다', () => {
    ...
    const run = (levels: readonly BaseLevelDef[]): BotResult[] =>
      SEEDS40.map((seed) => runBot(makeBotSimFor(stage, seed, STAGE1_DECK, 0, false, levels), stage, opts));
    ...
    // (실측 최악 생산비 1.215 · 봉쇄비 1.420. 열이 평탄해지면 둘 다 1.000이 된다)
