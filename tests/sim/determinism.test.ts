...
describe('결정론', () => {
   ...
   *                  **자격을 얻는 틱**이 다르면 일꾼이 다음에 어디로 가는가(pickAutoCell)와
   *                  그 칸에 지을 수 있는 마지막 틱이 다르다. 1틱만 어긋나도 타워 한 기의
   *                  자리가 갈리고, 그 발산은 몇 백 틱 뒤 gold 로만 보인다.
   *                  ⚠ 재고 게이트(R7)가 들어온 뒤 이 값이 지는 짐이 둘 더 늘었다:
   *                    **재생 선택 순서**가 `(regrowAt, 셀 키)` 이고, **웨이브별 지연 차이**도
   *                    텄던 틱에 여기 굳는다(`view.waveIndex` 는 hash() 에 안 접혀 있다).
