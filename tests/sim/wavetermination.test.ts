 ...
 *
 * 실측 최장: **2,801틱(93초, s1 w50)** — 2.14배 여유.
 * (s1 2801 · s2 2299 · s3 2469 · s4 1721 · s5 2789 · s6 2301)
 *
 * ⚠ 11단계(문간 교전)가 이 수를 2,772 → 2,801 로 **+29틱** 움직였다. 이론 상계는
 *   개체당 `GATE_HOLD_MAX_TICKS` = 360 이라 2,772 + 360 = 3,132 인데(gate.ts 보조정리 C),
 *   실제로는 웨이브의 끝을 정하는 것이 **가장 느린 보행자**이지 문 앞에 오래 서는
 *   trex 가 아니라서 증분이 그보다 훨씬 작다. 상계와 실측을 둘 다 적어 두는 이유는
 *   경로·속도 데이터가 바뀌면 실측은 움직여도 **상계는 안 움직이기** 때문이다.
 *
