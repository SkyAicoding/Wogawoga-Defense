...
export const CONTROLS: readonly Control[] = [
  ...
  {
    id: 'gather-x4',
    why: '짐값 기준값을 32로 (배포본 3의 10.7배, 스테이지1 판당 총액 494 → 5,238) — 채집이 실제로 지배 전략이 되는 세계',
    grade: 'kill',
    /**
     * ⚠ **[18] 의 계약 다리 일곱을 전부 겨냥한다.** 신설 항목이라 "겨냥을 좁게 적는 것 자체가
     *   커버리지 공백"이라는 이 파일의 교훈을 처음부터 지킨다.
     * ⚠ 아래 실측은 **짐값 8 · 재생 없던 세계**에서 잰 이력이다(그 트리에 재현할 창이 없다).
     *   재생 라운드가 배포본 짐값을 3 으로, 판당 총액을 494 로 내렸으므로 **이 팔의 배율은
     *   4배가 아니라 10.7배**이고 총액은 5,238 이다 — 곧 겨냥은 그때보다 **더 세게** 걸린다.
     *   숫자 32 자체는 한 자리도 안 건드렸다: 이 팔이 재는 것은 배율이 아니라 "짐값 축을
     *   충분히 밀면 계약 일곱이 전부 깨지는가"이고, 배포본이 내려간 만큼 그 답은 더 확실해진다.
     *   옛 실측(full, 짐값 32 · 재생 없음):
     *     · `18.rateCap`        판당 배달+손실 2,978 대 당시 문턱 745
  ...
  {
    id: 'gather-regrow-x3',
    why: '칸당 재생 횟수 상한 1 → 3 (스테이지1 총액 494 → 920) — 재생이 총액을 여는 세계',
    grade: 'kill',
    /**
     * ⚠ **재생 축의 양성 대조군.** `gather-x4` 는 **짐값** 축을 걸어 올리는데, 재생이 들어오면서
     *   총액에는 축이 **둘**이 됐다(총액 = Σ v × (1 + R)). 축 하나만 대조군을 가지면 나머지
     *   축의 회귀는 어떤 다리도 못 잡는다 — 재생 상한을 3으로 올려도 짐값은 배포본 그대로라
     *   `gather-x4` 는 이 세계를 한 번도 안 밟는다.
     * ⚠ `18.rateCap` 은 `s1GatherField()` 가 **배포본 값에서만** 총액을 뽑으므로(패치를 안 태운다)
     *   여기서 실제로 빨개진다. 따라갔다면 자기 자신을 재는 항등식이 되어 절대 안 빨개진다.
     * ⚠ `patch.id` 를 반드시 다르게 준다 — `envelope.playKey` 가 id 를 접는다.
     */
    targets: [
      '18.rateCap',
      '18.harvests',
    ...
    patch: { id: 'gather-regrow-x3', why: '칸당 재생 횟수 상한 3 (배포본 1의 세 배)', gather: { regrowMax: 3 } },
  },
  {
    id: 'gather-regrow-off',
    why: '칸당 재생 횟수 상한 0 — 다 캔 칸이 영영 안 자란다 (= 재생이 붙기 전의 세계)',
    grade: 'kill',
    /**
     * ⚠ **음성 대조군이다.** 겨냥은 **전제 하나**(`18.gatherHappens` 의 재생 조각)뿐이고,
     *   나머지 [18] 다리와 봉투 전체가 **초록으로 남는 것이 이 팔의 산출물**이다:
     *   *"재생 없는 세계로 되돌려도 아무 계약이 안 깨진다"* = 재생은 총액을 열지 않았다.
     *   (총액이 494 → 329 로 **내려가는** 방향이라 계약 다리는 전부 더 넓어진다.)
     * `gather-off`(전 종 gatherPct 0)와 짝이다 — 그쪽은 채집 자체를, 이쪽은 재생만 끈다.
     */
    targets: ['18.gatherHappens'],
    patch: { id: 'gather-regrow-off', why: '칸당 재생 횟수 상한 0', gather: { regrowMax: 0 } },
  },
  {
    id: 'gather-off',
