...
describe('wavegen', () => {
  for (const stage of STAGES) {
    describe(`stage ${stage.id}`, () => {
      ...
      /**
       * ── 11단계: **스테이지1의 마지막 웨이브는 진짜 봉우리여야 한다** ────────────
       *
       * 왜 스테이지1만 따로 잠그는가: 위 가드(> 1.1)는 전 스테이지 공통 하한이라
       * **거짓 주석을 통과시키고 있었다**. stage01의 w50에는 "클라이맥스 보정: w49 대비
       * 총 HP ≥1.15×"라고 적혀 있었는데 실측은 **1.1073**이었다 — 여유 0.0073으로 통과 중.
       * 곧 그 웨이브는 클라이맥스가 아니라 "조금 큰 일반 웨이브"였고, 실측에서도
       * 최강 봇 40판 중 22판이 **기지 체력을 한 톨도 잃지 않고** 게임을 끝냈다.
       *
       * 스테이지1에만 거는 이유는 여기가 **유일하게 봇이 완주하는 스테이지**이고
       * (autoplay 봉투) 실제로 플레이어가 끝까지 보는 유일한 판이기 때문이다.
       * 나머지 스테이지의 w50 비(1.13~1.41)도 같은 병을 앓고 있지만, 그쪽은 완주 실측이
       * 없어 값을 유도할 근거가 없다 — 근거 없이 숫자만 올리지 않는다.
       *
       * 문턱 1.5: 실측 1.993이고, w40(1.747)보다도 확실히 커야 "마지막이 가장 세다"가
       * 된다. 되돌리기(trex hpMul 2.4 → 1.38 · raptor 20 → 16 · boar 삭제)를 하면
       * 1.107로 떨어져 즉시 빨개진다.
       */
      if (stage.id === 1) {
        it('stage 1: w50이 앤티클라이맥스가 아니다 (직전 대비 ≥1.5배 · w40보다 크다)', () => {
          const r50 = totalHp(waveFor(50)) / totalHp(waveFor(49));
          const r40 = totalHp(waveFor(40)) / totalHp(waveFor(39));
          expect(r50, `w50 HP비 ${r50.toFixed(4)}`).toBeGreaterThanOrEqual(1.5);
          expect(r50, `w50 ${r50.toFixed(4)} / w40 ${r40.toFixed(4)}`).toBeGreaterThan(r40);
        });
      }
