...
describe('stages', () => {
  ...
  for (const stage of STAGES) {
    describe(`stage ${stage.id} (${stage.biome})`, () => {
       ...
       *   `data/stages/stage01.ts` 하나라, 나머지 다섯 판은 바로 아래 `if (!air) return;`
       *   에서 즉시 빠져나가 **무조건 초록**이다 — 여섯 개의 it 중 다섯이 아무것도 안 지킨다.
       *   그 다섯을 지금 억지로 살릴 방법은 없다. 이건 계기의 결함이 아니라 **콘텐츠가 아직
       *   없다**는 사실의 반영이고, s2~s6 에 하늘길이 들어오는 순간 **같은 어서션이 저절로**
       *   여섯 판에서 돌기 시작한다. 그래서 어서션은 손대지 않고 이 문장을 기록으로 남긴다.
       *   (초록 대신 '건너뜀'으로 보이게 하고 싶으면 `it.skipIf(!stage.airPaths)` 한 줄이면
       *    된다 — 어서션은 한 글자도 안 바뀐다. 지금은 일부러 안 했다: 이 라운드는 렌더 전용이다.)
       */
      it('공중 레인: 가장자리에서 시작해 화덕에서 끝나고, 지상보다 짧다', () => {
        const air = stage.airPaths;
        if (!air) return; // ← s2~s6 은 여기서 끝난다 (위 ⚠ 참조)
