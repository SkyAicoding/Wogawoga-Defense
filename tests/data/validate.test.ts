...
describe('towers', () => {
  ...
  /**
   * ⚠⚠ **이 계약은 2026-08-27 에 뒤집혔다** — 옛 문턱은 성장률 **1.9~2.1**(= 티어마다
   *   두 배)이었고, 그 규칙 자체가 사용자 지적으로 폐기됐다:
   *     > "강화할 때 200, 400, 800 이렇게 올라가는데 이게 너무 심해. 처음 생산은 90 이고
   *     >  이걸 강화 하는데 200이면 차라리 2개 세우는게 더 나은거 아냐?"
   *
   * 지적이 실측으로 맞았다. 강화 한 번이 주는 Δdps 를, **같은 돈으로 타워를 하나 더 세워
   * 얻는 dps** 와 나눈 값(= 아래 `가성비`)이 옛 표에서는 이랬다:
   *     T1→T2 0.30~0.34 · T2→T3 0.25~0.30 · T3→T4 0.16~0.23 · T4→T5 0.16~0.20
   * 곧 **강화는 증설의 1/3~1/6짜리 거래**였고, 티어가 오를수록 더 나빠졌다 — 칸이 남는
   * 한 강화할 이유가 없다는 뜻이라 티어 시스템 자체가 죽은 자산이었다.
   *
   * 새 표의 유도(값을 고른 게 아니라 **계산했다**): 강화비를 `Δdps ÷ (T1dps/T1비용)`
   * (= 파리티가)에 맞추면 전 11종에서 T1비용의 **0.63 / 1.07 / 1.70 / 2.83배**가 나온다.
   * 거기에 강화만 주는 것 — **사거리 증가**(창던지기 2.6→4.0, 면적 2.37배)와 **칸을 안
   * 먹는다** — 의 몫으로 약 1.5배를 얹으면 `T1비용 × 1.6^(n−1)` 이 된다. 그래서
   * **첫 강화값 = 배치값**이고 그 뒤로 1.6배씩이다(사용자의 직관과 정확히 같은 자리다).
   *
   * 문턱을 푼 것이 아니라 **재는 것을 바꿨다**. 옛 계약은 "곡선의 모양"만 봤고 그것이
   * 게임에서 뜻하는 바를 안 봤다 — 이 저장소가 반복해서 당한 꼴이다(CLAUDE.md §세 번 당한 병).
   * 지금은 **가성비를 직접 잰다**: 옛 표를 되돌리면 최악 0.161 이라 아래 문턱 0.45 에서
   * 즉시 빨개진다(옛 계약은 새 표에서 빨개졌지만, 새 계약은 옛 표에서도 빨개진다).
   */
  it('첫 강화값 = 배치값, 그 뒤 1.6배씩', () => {
    for (const id of EXPECTED_TOWERS) {
      const tiers = TOWER_DEFS[id].tiers;
      // ① 첫 강화(T1→T2)는 **타워를 하나 더 세우는 값과 같다** — 이 설계의 출발점이다
      expect(tiers[1]!.cost, `${id} 첫 강화값 = 배치값`).toBe(tiers[0]!.cost);
      // ② 그 뒤는 1.6배씩 (5의 배수 반올림 때문에 ±0.02 흔들린다)
      for (let i = 2; i < tiers.length; i++) {
        const ratio = tiers[i]!.cost / tiers[i - 1]!.cost;
        expect(ratio, `${id} T${i} cost 성장`).toBeGreaterThanOrEqual(1.5);
        expect(ratio, `${id} T${i} cost 성장`).toBeLessThanOrEqual(1.7);
      }
    }
  });

  /**
   * **강화가 증설보다 지나치게 나쁜 거래가 되면 안 된다** — 위 유도가 지키려는 것 자체.
   *
   * 가성비 = (강화가 주는 Δdps ÷ 강화비용) ÷ (T1 dps ÷ T1 비용).
   * 1.0 이면 "강화 한 번 = 타워 하나 더"와 정확히 같은 값이고, 지금 실측은 **0.51~0.76**이다
   * (최악 rattletrap T3 0.505). 1.0 을 안 노리는 이유는 강화가 **사거리와 칸 절약**을
   * 덤으로 주기 때문이다 — 그 몫이 프리미엄 약 1.5배이고 그래서 가성비가 1보다 낮다.
   *
   * ⚠ 문턱 0.45 는 실측 최악(0.505)보다 **아래**로 잡았다. 문턱을 실측에 딱 붙이면
   *   dmg 를 한 자리 손보는 것만으로 빨개져 잣대가 아니라 족쇄가 된다. 그러면서도 옛
   *   표(최악 0.161)와는 2.8배 떨어져 있어 되돌림은 확실히 잡는다.
   * ⚠ 버프·억제 전용(drum · hushtotem)은 dmg 가 0 이라 이 축으로 못 잰다. 그 둘은 위
   *   곡선 계약이 **같은 비율**을 강제하므로 다른 타워와의 상대 위치가 유지된다.
   */
  it('강화 가성비가 증설의 45% 아래로 떨어지지 않는다', () => {
    for (const id of EXPECTED_TOWERS) {
      const tiers = TOWER_DEFS[id].tiers;
      const t0 = tiers[0]!;
      if (t0.dmg === 0) continue; // 버프 전용 — 위 ⚠
      const perGold = t0.dmg / t0.cooldownTicks / t0.cost;
      for (let i = 1; i < tiers.length; i++) {
        const cur = tiers[i]!;
        const prev = tiers[i - 1]!;
        const dDps = cur.dmg / cur.cooldownTicks - prev.dmg / prev.cooldownTicks;
        const value = dDps / cur.cost / perGold;
        expect(value, `${id} T${i} 강화 가성비 (증설 대비)`).toBeGreaterThanOrEqual(0.45);
      }
    }
  });
...
describe('아군 값 (영구 유닛이 된 뒤의 하한)', () => {
   ...
   *   아군 실비용    = **gatherer 70** · clubber 90 · slinger 110 · guardian 160
   * 곧 지금 값은 하한에 붙어 있지 않다 — 이 문턱은 "적정값"이 아니라 **소모품 시절의
   * 가격표로 되돌아가는 것**을 막는 난간이다(40은 45 아래라 즉시 빨개진다).
   *
   * ⚠ **"전부 하한의 2배 이상"은 더 이상 참이 아니다** — 채집꾼 70은 하한 45의 **1.56배**다.
   * 이 문장이 거짓이 된 것은 어서션이 느슨해서가 아니라 **채집꾼이 전투재가 아니기 때문**이다:
   * 셋의 값은 "영구 전력"이라 타워와 같은 값대에 있어야 했지만, 채집꾼은 dmg 3 · blocks false 라
   * 전력이 아니라 **노동력**이고 그 값대는 타워가 아니라 **한 짐**(짐값 5~25)이 정한다.
   * 70이 더 내려가면 정원 2를 첫 판에 채우는 값(144)이 시작 골드 300 대비 너무 싸져
   * "채집꾼 둘 + 방어 0"이 무비용 갈래가 된다 — 그래서 아래 어서션(> 45)은 그대로 두되
   * 이 주석의 "2배" 문구는 **셋에만 해당한다**고 못 박는다. (docs/gather-spec.md §2)
   *
