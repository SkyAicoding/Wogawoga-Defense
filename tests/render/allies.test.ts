...
describe('아군 공격 동작 — 쿨다운 잔여 틱에 물린 한 번의 타격', () => {
  ...
  it('④ 무기가 벼르는 자세에서 치켜들었다가 타격에 앞아래로 떨어진다', () => {
    for (const id of ALL_ALLY_IDS) {
       ...
       *
       * ⚠ 주술사(옛 파수꾼)는 0.315 → 0.703 으로 뛰었다. 손에 든 것이 짧은 돌도끼
       * (어깨 기준 0.166)에서 **뼈 갈래 지팡이**(0.631)로 바뀌었기 때문이다 —
       * 지렛대가 3.8배가 되면서 벼르는 각도(back)는 오히려 1.25 → 0.70 으로 얕아졌다
       * (meshlib/enemies.ts ALLY_ATTACKS 3)번: 긴 자루를 1.25 로 젖히면 통째로
       * 등 뒤로 넘어간다).
       *
       * ⚠ 채집꾼이 이 어서션의 최저 여유(43%)를 쥔 종이다. 캐기 포즈는 각도가 얕고
       * (back 0.70) 뒤지개도 무기가 아니라 짧은 도구라, 낙차를 벌 손잡이가
       * **지렛대 길이**밖에 없다. 뒤지개를 줄이면 여기가 먼저 빨개진다
       * (meshlib/enemies.ts kitGatherer: MAIN 최원거리 0.352 — 넷 중 가장 짧다).
       */
