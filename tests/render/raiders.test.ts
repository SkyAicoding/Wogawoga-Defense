...
describe('부족 습격대 렌더', () => {
  ...
  it('삼각형 예산: 단품 400~700, 공유본은 그 종들 합의 절반 미만', () => {
     ...
     * 파수꾼→주술사 개편 뒤(지팡이 kit): 아군 4벌 **982** / 2,422 = **0.41**.
    ...
    /*
     * 실측 — 단품 clubber 578 / slinger 610 / **guardian 622** / gatherer 612, 공유본 **982**.
     * 아래 주석의 "제복을 변형 0으로 내렸다"가 되돌려지면 1,246으로 튄다.
     *
     * guardian 이 588 → 622 (+34) 로 오른 것은 **파수꾼이 주술사가 되면서**다:
     * 큰 세로 방패(46) → 작은 방패(32) · 짧은 돌도끼(28) → 뼈 갈래 지팡이(76).
     * 공유본은 948 → 982 로 34 늘어 이 1000 문턱 아래 **18** 을 남긴다. 다음에 아군
     * 장비를 얹을 사람은 그 18 을 먼저 보라 — 이 문턱은 "제복 4벌 복제 시절로
     * 되돌아갔나"를 잡는 덫이지 예산 자체가 아니고(절대 상한은 아래 1250),
     * 넘길 거면 최악 프레임을 실제로 재고 근거를 여기 남겨야 한다.
     */
