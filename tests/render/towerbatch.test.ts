 ...
 *
 * (2라운드 2-c 로 타워가 8 → **11종**이 됐다. 이 파일의 조합 수는 `ALL_TOWER_IDS` 에서
 *  유도하므로 종이 더 늘어도 안 낡는다 — 한때 40 이 박혀 있다가 낡았다.)
 *
 * ── 이 파일이 지키는 주장 ──────────────────────────────────────────────────
...
describe('타워 드로우콜 상한', () => {
  ...
  it('(종,티어) 조합을 다 세워도 상한이 유지된다', () => {
    // 전 종 × 5티어를 한 판에 전부 — 예전 구조에서 최악(=조합마다 메시가 갈리는) 경우다
    // ⚠ 기대값을 **베끼지 않고 유도한다.** 2-c 로 타워가 8 → 11 종이 되자 여기 박혀 있던
    //   40 이 낡아 빨개졌다 — 상수를 베끼면 언젠가 반드시 갈라진다(CLAUDE.md 처방).
    const COMBOS = IDS.length * 5;
    const s3 = build(STAGES[0]!, Q);
    let i = 0;
    for (const id of IDS) {
      for (let t = 0; t < 5; t++) {
    ...
    expect(i).toBe(COMBOS);
    expect(d.calls - base, `${COMBOS}조합: ${base} → ${d.calls}콜`).toBeLessThanOrEqual(6);
  });
