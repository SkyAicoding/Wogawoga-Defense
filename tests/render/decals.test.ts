 ...
 * 그리고 ②의 반대쪽도 함께 잠근다: **이동 목표 표식의 기둥은 남아야 한다**.
 * 그쪽은 빈 길 위의 한 점이라 링만으로는 안 보이고 하단 패널에 덮이기도 한다 —
 * "기둥을 없애라"를 showSortieMarker까지 밀고 가면 그 표식이 안 보이게 된다.
 *
...
describe('Decals — 부족 공격 사거리 바운더리', () => {
  ...
  it('②의 반대쪽 — **이동 목표 표식의 기둥은 남는다** (빈 칸 위의 한 점이라 링만으론 안 보인다)', () => {
    const { scene, decals } = make();
    decals.showSortieMarker([{ x: 4, z: 4 }]);
    const s = stats(visibleMeshes(scene)[0]!, 4, 4);
    expect(s.yMax, '봉수대 기둥이 사라졌다 — 선택 표시와 목표 표식을 같이 지웠다').toBeGreaterThan(2);
  });
