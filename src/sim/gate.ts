 ...
 *   | 주술사 힐 오라 | ⚠ 문간 좌표가 1.85~2.99 라 `healAura.radius = 2` 안에 **서로가 다
 ...
 *   (스폰→정지선, 오늘과 동일) + (체류 ≤ 상한) − (오늘 마지막 1.85~2.99타일을 걷던 시간).
 *   곧 개체별 증분 ≤ 상한이고, 웨이브 종료는 개체 퇴장의 최댓값이므로 웨이브 길이 증분도
 *   ≤ 상한이다. ∎
 *
 * 【계량】 같은 국면의 **실측 최장 웨이브 3,141틱**(s1 w50), 상한 6,000 → 여유 **1.91배**.
 *   옛 주기(30 · 체류 상한 360)의 2,801틱에서 +340틱이다 — 개체당 상한이 +360 인데
 *   웨이브 증분이 그보다 작은 것이 보조정리 C 가 상계이고 등식이 아니라는 뜻이다.
 *   (상한이 360 → 720 으로 간 이유는 `GATE_BITE_TICKS` 가 30 → 60 이 됐기 때문이다 —
 *    상한은 언제나 `최대 baseDamage × 주기` 라는 **종속 변수**다. balance.ts 참조)
 *
