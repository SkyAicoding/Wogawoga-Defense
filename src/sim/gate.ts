 ...
 *    곧 개체 중심은 마을 중심에서 정확히 `1.45 + e.radius` 다.
 *    반경을 더하는 이유는 **몸 앞끝을 한 선에 세우기 위해서**다. 이 한 줄이 셋을 산다:
 *      · 앞끝 1.45 = `render/meshlib/basecamp.ts` 의 `BASECAMP_MAX_RADIUS` →
 *        **마을 바깥끝에 정확히 닿아 서고 한 톨도 안 들어온다.**
 *        ⚠⚠ 옛 값 1.15 는 여기서 **비교 대상을 틀렸다**: 1.0 은 구조물이 놓인 **중심
 *          고리**이지 바깥끝이 아니고(목책 `WALL_R` 만 해도 1.28), 그래서 적 메시가
 *          움막과 목책을 뚫고 서 있었다. `tests/sim/gate.test.ts` 의 잣대가 같은 착오를
 *          베껴 중심 고리를 재고 있어 **테스트는 초록인데 그림이 틀렸다.** 값과 잣대를
 *          둘 다 바깥끝으로 옮겼다.
 *      · 최대 중심거리 2.25(trex, radius 0.80) < **마을 Lv1 사거리 2.3** →
 *        16종 전부가 첫 레벨 마을의 사거리 안이다. "홈타운이 적을 공격한다"가 성립한다.
 *        ⚠ Lv1 사거리는 이 값과 **함께** 2.0 → 2.3 으로 갔다. 1.45 를 두고 2.0 을
 *          그대로 두면 mammoth(2.07)·spino(2.15)·trex(2.25) 세 종이 사거리 밖이다.
 ...
 *      1.67~2.25(= `GATE_STANDOFF_EDGE` + 반경)로 나가면서, 마을 쪽으로 깊어지던 옛
 ...
 *   | 주술사 힐 오라 | ⚠ 문간 좌표가 1.67~2.25 라 `healAura.radius = 2` 안에 **서로가 다
