 ...
 *    35% 감속이면 그 대상이 타워에 넣는 피해도 35% 준다.
 *
 *    ⚠ **여기 있던 한 문장을 뒤집는다 (11단계 · 문간 공성).** 원문은
 *      "기지 피해(누수)는 공격이 아니라 도달로 계산되므로 이 규칙과 무관하다 —
 *       일관성 문제도 생기지 않는다."
 *    였고 그때는 참이었다. `leakEnemy`는 경로 끝에 **닿았다는 사실**만 보고 `baseDamage`를
 *    한 번 깎았으므로, 거기에는 곱할 '한 대의 위력'이 없었다.
 *    지금은 거짓이다: 보스는 경로 끝에서 사라지지 않고 문간에 서서 GATE_BITE_TICKS마다
 *    **문다**(src/sim/gate.ts). 한 입은 도달이 아니라 주기적인 공격이고, 그래서 이 규칙이
 *    그대로 걸린다 — `gate.bite`가 `slowFactor(e)`를 곱한다(gate.ts 규칙 5).
 *    곧 규칙 9의 적용 범위는 이제 **"적이 때리는 모든 것"**이다: 타워도, 마을도.
 *    반대로 두면 오히려 일관성이 깨진다 — 같은 얼어붙은 팔이 움막은 약하게 치고 마을은
 *    온전히 무는 그림이 되고, "얼음은 습격대 앞에서 죽은 카드가 아니다"라는 이 규칙의
 *    목적이 문간에서만 예외가 된다.
 *    ⚠ 아직 참인 부분: **보스가 아닌 적의 누수**는 여전히 도달로 계산되고 감속과 무관하다.
 *      문간에 서는 것은 보스뿐이기 때문이다(gate.ts 규칙 1).
 *
