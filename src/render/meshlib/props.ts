 ...
 *    질량을 **위에 몰고 밑동을 비운다** — 거목은 줄기만 1.78까지 맨몸으로 올라가고
 *    캐노피가 그 위에 있어, 그 밑을 지나는 적·타워·체력바는 그대로 보인다. 아치는
 *    아예 구멍이 뚫려 있고, 얼음 첨탑과 고사목은 살만 남은 골격이다.
 *
 *    ── 상한을 실측으로 잡았다 (tests/render/props.test.ts "가림 실측") ──────
 *    각 삼각형을 카메라 반대로 지면에 투영해 마스크를 만들고, 유닛 반지름(0.25셀)
 *    만큼 침식한 뒤 남는 면적 = **유닛 하나가 통째로 숨을 수 있는 넓이**를 잰다.
 *    (단순 면적으로는 답이 틀린다 — 가는 줄기 띠를 캐노피와 같은 값으로 세기 때문에
 *     palmColossus 가 palmTall 보다 25% 더 가린다고 나왔는데, 초과분 전액이 폭
 *     0.22셀짜리 줄기였다. 폭 0.22 띠는 지름 0.5인 적을 통째로 가리지 못한다)
 *    피치 55°/40°(하한, 가장 많이 가리는 각) 실측, 단위 셀²:
 *        초원 0.23/0.26 (1층 최악 0.44/0.58)  ·  정글 0.15/0.04 (0.20/0.23)
 *        사막 0.00/0.00 (0.25/0.36)           ·  설원 0.08/0.12 (0.22/0.26)
 *        늪   0.00/0.00 (0.31/0.39)           ·  화산 0.37/0.51 (0.43/0.51)
 *    곧 **여섯 랜드마크 전부가 기존 1층 최악보다 덜 가린다**. 높이를 1.5배로 올리고도
 *    게임플레이 가림은 오히려 줄었고, 그게 "아래를 비운다"가 값을 하는 지점이다.
 *    사막 아치와 늪 고사목은 0.00 — 그 뒤에서는 아무것도 사라지지 않는다.
 *    이 표가 곧 상한이다: 새 랜드마크는 같은 테스트를 통과해야 하고, 통과 못 하면
 *    키를 줄이는 게 아니라 **밑동을 더 비워야** 한다.
...
/**
 * 설계도를 구운 지오메트리 (테스트/계측용 — 호출자가 dispose 한다).
 * 랜드마크가 게임플레이를 얼마나 가리는지 재려면 실루엣이 필요하고, 실루엣은
 * pos/scale 표가 아니라 구운 삼각형에만 있다 (elementHeight 와 같은 이유).
 */
export function elementGeometry(el: Element): THREE.BufferGeometry {
  return bakeElement(el, 1);
}
