...
function kitRaw(biome: BiomeId): GdKit {
  ...
  const g = pal.ground;
  /*
   * 램프를 **휘도순으로 정렬해** 쓴다. 팔레트 주석이 "ground 배열의 순서는 뜻이 없다"고
   * 못박아 뒀는데도 여기서는 g[0]/g[2]/g[4] 를 인덱스로 꺼내 쓰고 있었고, 그래서
   * "밝은 쪽 얼룩"이 바이옴마다 램프의 아무 색이나 잡았다. 사막에서는 그게 이미
   * 램프 최상단인 0xf9de95 였고 거기에 ×1.09 를 걸어 r·g 채널이 255에서 잘려
   * **채도가 빠진 흰 얼룩**이 됐다 — 확대 캡처에서 모래 위 흰 종잇조각으로 읽혔다.
   * 정렬해서 뽑으면 밝게 만들 색으로 램프의 **어두운 쪽**을 골라 클리핑을 피할 수 있고,
   * 폭은 뒤의 stretchRamp 가 HSL 명도로 벌리므로 채도가 안 빠진다.
   */
  const ramp = [...g].sort((a, b) => gdLuma(a) - gdLuma(b));
  const gLo = ramp[0] ?? 0x808080;
  const gMid = ramp[ramp.length >> 1] ?? gLo;
  const gHi = ramp[ramp.length - 1] ?? gLo;
  const g0 = gLo;
  const g2 = gMid;
  ...
  /*
   * 면 얼룩 색은 전부 **팔레트 램프에서 유도**한다 (직접 하드코딩하지 않는다).
   *
   * 명도 배율 폭을 0.93~1.05 에서 0.88~1.06 으로, 액센트색 혼합을 0.24 에서 0.20~0.45 로
   ...
   * 정확히 설원이었다.
   * ⚠ **1을 넘는 배율은 쓰지 않는다.** shade() 는 채널을 255에서 자르므로 밝히면
   *   채도가 빠지고, 사막처럼 램프 최상단이 이미 249인 바이옴은 흰색이 된다
   *   (확대 캡처에서 모래 위 흰 종잇조각으로 읽혔다). 밝은 쪽은 램프 최상단을
   *   **그대로** 쓰고, 폭은 뒤의 stretchRamp 가 HSL 명도로 벌린다 — 그쪽은 h·s 를
   *   고정하므로 아무리 벌려도 채도가 안 빠진다.
   * ⚠ 반대로 "정렬했으니 밝은 색을 어둡게, 어두운 색을 밝게"로 뒤집는 것도 틀렸다.
   *   한 번 그렇게 뒀더니 다섯 색이 한 점으로 모여(초원 폭 0.04) 스트레치 상한 4배로도
   *   0.16밖에 못 벌렸다. 유도 단계는 **벌려 놓고**, 스트레치는 거기서 더 벌린다.
   */
  const inner = [
    shade(gLo, 0.88),
    gHi,
...
export function buildGroundDetail(
  ...
  const bakeCell = (x: number, z: number): THREE.BufferGeometry | null => {
    ...
    for (const st of blotSites) {
      ...
      /*
       * y 는 사이트마다 다르게 내린다(겹칠 때 z-파이팅 방지, Y_BLOT 주석 참고).
       * 밝기 흔들기는 **사이트 해시**에서 뽑고 폭도 ±2.5%로 좁힌다. 셀 rng 로 뽑으면
