 ...
 *
 * factory.ts 는 읽기 전용이므로 조립 헬퍼는 전부 이 파일 안에 둔다.
 */
...
function raptor(): PartSpec[] {
  ...
  const tail = arc([-0.24, 0.45, 0], [-0.5, 0.5, 0], [-0.8, 0.34, 0], 8);
  const neck = arc([0.2, 0.49, 0], [0.36, 0.6, 0], [0.42, 0.7, 0], 4);
  return [
    ...tubeW(torso, [0.24, 0.3, 0.3, 0.25], body, { flat: 1.02, hueJitter: 0.014 }),
    ...tube(tail, 0.19, 0.035, body, { kind: 'cyl', seg: 6, flat: 1.15, hueJitter: 0.012 }),
    ...
    // 등~꼬리 깃털 라인
    ...spines(
      [[0.1, 0.55, 0], [-0.02, 0.55, 0], [-0.14, 0.53, 0], [-0.3, 0.52, 0], [-0.46, 0.5, 0], [-0.62, 0.45, 0]],
      0.1,
      0.05,
      0.035,
      dark,
    ...
    // 다리 + 낫발톱
    ...legBiped(0.0, 0.13, 0.42, 0.088, body, dark),
    ...mirZ([
...
function compy(): PartSpec[] {
  ...
  const tail = arc([-0.11, 0.24, 0], [-0.3, 0.28, 0], [-0.52, 0.19, 0], 7);
  return [
    ...tubeW(torso, [0.13, 0.17, 0.17, 0.13], body, { flat: 1.0, hueJitter: 0.02 }),
    ...tube(tail, 0.1, 0.02, body, { kind: 'cyl', seg: 5, flat: 1.15 }),
    ...tube(arc([0.13, 0.26, 0], [0.2, 0.32, 0], [0.22, 0.37, 0], 3), 0.09, 0.075, body, { kind: 'cyl', seg: 5 }),
    ...
    // 등 볏
    ...spines(
      [[0.08, 0.3, 0], [0.0, 0.3, 0], [-0.08, 0.29, 0], [-0.17, 0.28, 0], [-0.26, 0.27, 0]],
      0.07,
      0.035,
      0.028,
      dark,
    ...
    ...legBiped(0.0, 0.07, 0.21, 0.05, body, dark),
  ];
}
...
function trike(): PartSpec[] {
  ...
  const parts: PartSpec[] = [
    ...
    // 꼬리
    ...tube(arc([-0.42, 0.44, 0], [-0.62, 0.42, 0], [-0.78, 0.28, 0], 6), 0.24, 0.045, body, {
      kind: 'cyl',
      seg: 6,
      flat: 1.1,
    }),
    // 목 + 머리
    ...tube([[0.32, 0.48, 0], [0.44, 0.5, 0]], 0.36, 0.32, body),
    ...
    ...legQuad(0.22, 0.22, 0.3, 0.15, body, dark, 0.04),
    ...legQuad(-0.3, 0.22, 0.3, 0.16, body, dark, -0.04),
  ];
  // 프릴 가장자리 뿔 7개
  for (let i = 0; i < 7; i++) {
...
function ptera(): PartSpec[] {
  ...
  return [
    ...
    ...tube(arc([-0.2, 0.35, 0], [-0.38, 0.34, 0], [-0.54, 0.27, 0], 4), 0.1, 0.025, body, { kind: 'cyl', seg: 5 }),
    ...
    // 꼬리 끝 방향타 + 몸통 줄무늬
    { kind: 'box', pos: [-0.52, 0.29, 0], rot: [0, 0, 0.35], scale: [0.14, 0.11, 0.03], color: 0xb84a2e },
    ...mirZ([
...
function ankylo(): PartSpec[] {
  ...
  const parts: PartSpec[] = [
    ...
    // 짧고 굵은 다리
    ...legQuad(0.24, 0.24, 0.2, 0.14, dark, 0x5f5232, 0.02),
    ...legQuad(-0.28, 0.24, 0.2, 0.15, dark, 0x5f5232, -0.02),
    // 꼬리 + 곤봉
    ...tube(arc([-0.38, 0.31, 0], [-0.54, 0.33, 0], [-0.68, 0.31, 0], 4), 0.17, 0.1, body, { kind: 'cyl', seg: 6 }),
...
function boar(): PartSpec[] {
  ...
  const parts: PartSpec[] = [
    ...
    ...legQuad(0.16, 0.14, 0.26, 0.09, dark, C.black, 0.03),
    ...legQuad(-0.24, 0.13, 0.22, 0.085, dark, C.black, -0.03),
    // 갈라진 발굽
    ...mirZ([
    ...
    ...mirZ([
      ...
      { kind: 'box', pos: [0.34, 0.45, 0.07], rot: [0, 0, -0.3], scale: [0.05, 0.05, 0.05], color: 0x74492c },
    ]),
    // 꼬리
    ...tube(arc([-0.36, 0.38, 0], [-0.46, 0.4, 0], [-0.48, 0.28, 0], 3), 0.045, 0.025, mane, { kind: 'cyl', seg: 4 }),
  ...
  for (let i = 0; i < 8; i++) {
    ...
    parts.push({ kind: 'cone', pos: [x, yTop + h * 0.35, 0], rot: [0, 0, -0.3 - t * 0.35], scale: [0.05, h, 0.05], color: mane, seg: 4 });
    if (i % 2 === 0) {
      parts.push(
...
function shaman(): PartSpec[] {
  ...
  const staffZ = 0.32;
  const parts: PartSpec[] = [
    // 로브 — 아래로 퍼지는 층 5단 (각진 원뿔대)
    { kind: 'cyl', pos: [0, 0.06, 0], scale: [0.5, 0.12, 0.5], color: robeDark, seg: 7, hueJitter: 0.018 },
    { kind: 'cyl', pos: [0, 0.2, 0], scale: [0.44, 0.18, 0.44], color: robe, seg: 7, hueJitter: 0.018 },
    { kind: 'cyl', pos: [0, 0.37, 0], scale: [0.36, 0.2, 0.36], color: robeDark, seg: 7, hueJitter: 0.018 },
  ...
  parts.push(
    ...
    link([0.18, 0.62, 0.24], [staffX, 0.6, staffZ], 0.06, 0.06, C.skin, { kind: 'cyl', seg: 5, pad: 0.02 }),
...
function mammoth(): PartSpec[] {
  ...
  const parts: PartSpec[] = [
    ...
    ...tube(arc([0.74, 0.76, 0], [0.96, 0.5, 0], [0.84, 0.1, 0], 7), 0.22, 0.07, fur, { kind: 'cyl', seg: 6, hueJitter: 0.012 }),
    ...
    // 다리 (굵고 짧음)
    ...legQuad(0.32, 0.27, 0.5, 0.22, furDark, 0x5f3d20, 0.03),
    ...legQuad(-0.44, 0.27, 0.5, 0.23, furDark, 0x5f3d20, -0.03),
    // 꼬리
    ...tube(arc([-0.62, 0.68, 0], [-0.78, 0.58, 0], [-0.8, 0.4, 0], 4), 0.09, 0.04, fur, { kind: 'cyl', seg: 5 }),
...
function theropod(c: TheroCfg): PartSpec[] {
  ...
  return [
    ...
    // 다리
    ...legBiped(-0.04 * s, 0.19 * s, 0.72 * s, 0.13 * s, body, dark),
    // 허벅지 근육 덩어리
...
function spino(): PartSpec[] {
  ...
  const s = 1.05;
  const V = (x: number, y: number, z = 0): V3 => [x * s, y * s, z * s];
  const parts = theropod({ body, belly, dark, s });
  ...
  for (let i = 0; i < 10; i++) {
    ...
    parts.push(
      { kind: 'box', pos: [x, y + h * 0.5, 0], scale: [0.085 * s, h, 0.055 * s], color: 0xe07a3a, hueJitter: 0.025 },
      { kind: 'cone', pos: [x, y + h + 0.035 * s, 0], scale: [0.075 * s, 0.09 * s, 0.05 * s], color: 0xf0a04a, seg: 4 },
    );
    if (i < 9) {
      parts.push({
        kind: 'box',
...
function trex(): PartSpec[] {
  ...
  // 등~꼬리 골판 (뒤로 갈수록 작아짐)
  parts.push(...spines(arc(V(0.26, 1.02), V(-0.3, 1.08), V(-0.96, 0.74), 10), 0.2 * s, 0.05 * s, 0.09 * s, dark, 4, -0.45));
  return parts;
}
...
function golem(): PartSpec[] {
  ...
  const cracks: Array<[V3, V3, number]> = [
    [[0.24, 0.74, 0.2], [-0.06, 0.58, 0.32], 0.06],
    [[-0.06, 0.58, 0.32], [-0.22, 0.44, 0.2], 0.05],
    [[0.12, 0.92, 0.18], [0.28, 0.76, 0.06], 0.055],
    [[0.02, 0.88, -0.26], [-0.16, 0.66, -0.32], 0.055],
    [[0.2, 0.52, -0.28], [0.3, 0.36, -0.14], 0.045],
    [[0.1, 0.72, 0.44], [0.14, 0.54, 0.5], 0.05],
    [[0.1, 0.72, -0.44], [0.14, 0.54, -0.5], 0.05],
    [[0.18, 0.4, 0.52], [0.22, 0.24, 0.55], 0.05],
    [[0.18, 0.4, -0.52], [0.22, 0.24, -0.55], 0.05],
    [[-0.06, 0.32, 0.26], [0.0, 0.16, 0.3], 0.05],
    [[-0.06, 0.32, -0.26], [0.0, 0.16, -0.3], 0.05],
    [[0.3, 0.6, 0.1], [0.34, 0.44, 0.02], 0.045],
  ];
  for (let i = 0; i < cracks.length; i++) {
    const [a, b, w] = cracks[i]!;
    parts.push(link(a, b, w, w, i % 2 === 0 ? C.lava : 0xffa042, { pad: 0.02 }));
  }
  // 등에 박힌 발광 결정
  for (let i = 0; i < 3; i++) {
    const a = -0.7 + i * 0.7;
