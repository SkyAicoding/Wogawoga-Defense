 ...
 * 창을 멘 blade·lancer 와는 **기운 방향**이 갈린다: 저쪽은 어깨 너머 뒤로(끝이 x −0.33
 * ~ −0.41), 이쪽은 앞으로(끝이 x +0.30) 기운다. 같은 대각선이 아니다.
 *
 * ── 원시시대 톤 ────────────────────────────────────────────────────────────
...
function kitGuardian(ids: RaiderIds): PartSpec[] {
  ...
  return [
     ...
     *
     * ⚠ **앞으로 기울인 것이 아트가 아니라 계약이다.** `allies.test.ts ④` 는 전 종에
     * `ready − hit > 0.2`(모델 키 0.77 의 1/4)를 요구하는데, 이 낙차는 어깨 피벗을
     * 도는 z축 회전이라 정점 하나의 기여가 `R·[sin(back+φ) − sin(fwd+φ)]` 로 닫힌
     * 형태다(R = 피벗까지 거리, φ = 피벗에서 본 각도). 파수꾼 각도(back 1.25 /
     * fwd −1.2)에서 이 값은 `1.882·R·cos(φ + 0.025)` 이므로 **φ 가 90°(머리 위)에
     * 가까울수록 0으로 죽는다** — 곧 **수직으로 세운 지팡이는 아무리 길어도 낙차를
     * 못 번다.** 실제로 수직(φ≈1.32)으로 굽던 시안은 낙차가 0.222 로 문턱 0.2 를
     * 겨우 넘었다(여유 11%, 채집꾼 43%보다 위태롭다).
     * 앞으로 19.6° 기울이면 갈래 끝이 φ≈1.02 로 내려와 실측 낙차가 **0.583**(291%)이 된다.
     ...
     * (hexer 주석의 "밑동은 y=0.055 위로"와 같은 함정이다).
     */
    ...tag(ids.armR, [
       ...
       */
      ...pairZ([link([0.288, 0.775, sz], [0.318, 0.938, -0.062], 0.048, 0.048, C.bone, { kind: 'cone', seg: 4, pad: 0.01 })], sz),
