 ...
 * 요 허용 폭 (기본 ±).
 *
 * ⚠ **40° → 52° 로 넓혔다 (사용자 요구, 2026-08-29)**:
 *   > "오른쪽 마우스로 땅을 잡고 상하로 움직이거나 좌우로 움직였을때 각도를
 *   >  지금 보다 30% 정도 더 증가 시켜줘. 더 많이 상하로 각도를 바꿔 보고 싶어"
 *   40 × 1.3 = 52. 곧 요는 [-87°, +17°] 를 돈다(전에는 [-75°, +5°]).
 *   드래그 감도(`battlecontroller.ORBIT_YAW_PER_PX`)도 같은 배율로 올려서, 넓어진
 *   폭을 **같은 드래그 거리**로 다 훑는다 — 폭만 넓히면 손이 그만큼 더 가야 한다.
 *
 * 옛 근거는 "넘어가면 길·기지 방향 감각이 깨진다" 였다. 그 감각은 취향 축이라
 * 사용자가 직접 보고 정할 자리이고, 실제로 더 보고 싶다고 했다.
 */
const YAW_RANGE = 52 * DEG;
/**
 * 피치 하한 **35.5°** (기본 55° 에서 −19.5°).
 *
 * ⚠ **40° → 35.5° 로 낮췄다 (위 YAW_RANGE 와 같은 사용자 요구).** 기본각에서의
 *   아래 여유 15° × 1.3 = 19.5°.
 *
 * ⚠⚠ **옛 하한 40° 에는 실측 근거가 있었다** — 캡처 pitchlow-34/36/38/40 으로
 *   38°부터 바다 평면의 먼 가장자리가 HUD 상단바 아래로 내려와 "수평선"처럼 보였다.
 *   새 하한 35.5° 는 그 선 아래다. 곧 **하한 끝까지 내리면 그 수평선 띠가 다시 보인다.**
 *   사용자가 "더 많이 상하로 각도를 바꿔 보고 싶다"고 했으므로 폭을 우선했고,
 *   보고 나서 되돌릴지는 사용자가 정한다. 되돌리려면 이 값만 40 으로 되돌리면 된다.
 */
const PITCH_MIN = 35.5 * DEG;
/**
 * 피치 상한 **68°** (기본 55° 에서 +13°).
 *
 * ⚠ **65° → 68° 로 올렸다 (같은 사용자 요구).** 위 여유 10° × 1.3 = 13°.
 *
 * ⚠⚠ **옛 상한 65° 에도 실측 근거가 있었다** — 캡처 pitch-64/66/68 비교에서
 *   68~70° 는 절벽 스커트가 거의 사라지고 나무 실루엣이 납작해져 로우폴리 입체감이
 *   죽었다. 새 상한은 그 구간의 **시작점**이다. 끝까지 올리면 탑다운에 가까워지고
 *   두께가 얇아진다 — 의도한 대가다. 되돌리려면 이 값만 65 로 되돌린다.
 */
const PITCH_MAX = 68 * DEG;
