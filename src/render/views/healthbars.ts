...
/**
 * 타워 바 높이 — **적 일반 바와 같은 0.11** (0.22 → 0.17 → 지금).
 *
 * ── 2026-08-28 : 0.17 → 0.11 (사용자 요구) ─────────────────────────────────
 *   > "우리 편 타워, 주민, 홈타운 등의 hp 바의 두께를 적군의 hp 두께 정도로 줄여줘.
 *   >  지금 우리편 hp 두께가 너무 두꺼워"
 *
 * ⚠⚠ **문턱을 내린 것이 아니라 설계 축이 바뀌었다.** 옛 주석은 "0.17 에서 멈춘다 —
 *   테스트가 `> 0.165` 를 잠근다"였고, 그 계약의 뜻은 *"적 바와 내 편 바가 시각적으로
 *   갈려야 한다"* 였다. 두께는 그 갈림을 만드는 **한 가지 수단**이었을 뿐이고, 진짜
 *   갈림은 `barKind` → 팔레트(내 편 파랑 계열 / 적 초록→빨강)다. 사용자가 두께 축을
 *   버리기로 했으므로 계약도 **뜻을 보존해 다시 썼다**(두께 비 대신 "내 편 바가 적 바보다
 *   두껍지 않다"를 잠근다 — 되돌리면 그쪽이 빨개진다).
 */
const TOWER_BAR_H = 0.11;
/**
...
/** 아군 바 높이 — **적 일반 바와 같은 0.11** (종전 0.15). 사람 크기 유닛이라 적과 같은 두께가 맞다 */
const ALLY_BAR_H = 0.11;
...
/**
 * 기지 바 높이 — 0.3 → 0.22 → **0.14** (사용자 요구, 위 `TOWER_BAR_H` 주석과 같은 건).
 *
 * 적 일반 바(0.11)보다 살짝만 두껍다. 기지를 **육중하게** 남기는 몫은 두께가 아니라
 * **폭(`BASE_BAR_W` 1.3)** 이 이미 지고 있다 — 폭이 타워 바의 1.4배·적 바의 2배 넘는다.
 * ⚠ 기지는 `barKind 4` 를 유지한다 — kind 1 로 합치면 저체력 점멸(패배 임박의 유일한
 *   경보)이 조용히 사라진다.
 */
const BASE_BAR_H = 0.14;
/**
...
if (vKind > 4.5) {
  ...
  /*
   * ── 테두리는 **적과 같은 두께**다 (사용자 요구: "우리 HP 표시하는 디자인을 적 hp
   *    표시 하는것 처럼 디자인을 간략하게") ────────────────────────────────────
   *
   * 종전에는 mix(0.06, 0.17, own) 이라 내 편 바만 테가 높이의 **34%**를 먹었다.
   * 그게 "간략하지 않다"의 정체였다 — 실측으로 옆 테두리가 적 0.26px 대 내 편 1.1px 라
   * 같은 크기의 바인데 하나만 액자에 든 것처럼 보였다. 그리고 그 대가로 **채움 띠가
   * 얇아진다**: 타워 바는 총 높이가 적의 두 배인데 실제로 보이는 색 띠는 2.98px 대
   * 1.98px 로 1.5배밖에 안 됐다. 곧 두꺼운 테는 정보를 더한 게 아니라 **가렸다**.
   *
   * ⚠ **own 을 지우면 안 된다.** 테 '두께'에서만 빠지는 것이고, 팔레트(의미 반전)와
   *   테 '색'(명도 대비)에서는 계속 쓰인다 — 아래 두 줄이 그것이다. 두께·눈금이라는
   *   구분 축 둘을 뺐으므로 남은 둘은 **반드시** 지켜야 한다. 특히 A~E 뒤에는 적 보스
   *   바(높이 0.17)와 타워 바(0.17)의 높이가 같아져서, 명도 대비가 유일한 즉시
   *   구분자가 된다.
   */
  float inset = 0.06;
  float edge = step(inset, vBarUv.y) * step(vBarUv.y, 1.0 - inset)
  ...
  /*
   * ⚠ 중앙 눈금(옛 tick)은 **뺐다.** 실측 폭이 0.44px 라 눈금으로 보이지 않고
   *   띠 한가운데 얼룩으로만 읽혔다 — 정보가 아니라 노이즈였다.
   */
  // 테 '색' 은 그대로 갈린다: 적은 검정, 내 편은 밝은 뼈/돌색. 두께 비용 0으로
  // "저건 내 것" 비트를 지키는 유일한 수단이다 (위 ⚠ 참조).
  vec3 frame = mix(vec3(0.04), vec3(0.93, 0.90, 0.82), own);
  diffuseColor.rgb = mix(frame, barCol, edge);
} else {
