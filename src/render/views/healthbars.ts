...
/**
 * 타워 바 높이 — **적 보스 바와 같은 0.17** (종전 0.22).
 * 테를 얇게 한 뒤(위 셰이더 A) 실제 채움 띠는 0.22×0.66 = 2.98px → 0.17×0.88 = **3.07px**
 * 로 **오히려 두꺼워진다.** 바는 작아지는데 색은 더 잘 보인다.
 * ⚠ 0.17 에서 멈춘다 — `tests/render/healthbars.test.ts` 의 "타워 바가 확실히 두껍다"가
 *   `> 0.165` 를 잠근다. 더 내리려면 그 계약을 다시 유도해야 하는데, 대체 축(팔레트·테
 *   색)은 프래그먼트 상수라 WebGL 없는 테스트로 되읽을 수 없다 = 실패 불가능한 계약이 된다.
 */
const TOWER_BAR_H = 0.17;
/**
...
/**
 * 기지 바 높이 — 0.3 → **0.22**. 테를 얇게 한 뒤 채움 띠는 4.06 → 3.97px 로 사실상 같다.
 * 기지를 육중하게 남기는 몫은 **폭(BASE_BAR_W 1.3)** 이 이미 진다.
 * ⚠ 기지는 `barKind 4` 를 유지한다 — kind 1 로 합치면 저체력 점멸(패배 임박의 유일한
 *   경보)이 조용히 사라진다.
 */
const BASE_BAR_H = 0.22;
/**
