...
export function run(): void {
  ...
  /**
   * 목 미리보기 — 랩에서 **손패 카드의 상성 경고**를 재현하기 위한 고정 편성이다.
   *
   * ⚠ 원래 목적은 없어진 미리보기 띠의 레이아웃(칩 상한 6개 · `+N` 접힘 · 배지 네 종류)을
   *   보는 것이었다. 띠가 사라진 지금 **7종을 넣은 이유는 아래 프테라 주석 하나뿐이다** —
   *   공중 비중이 AIR_BLIND_SHARE 를 넘겨 경고가 실제로 켜지는 편성을 만드는 것.
   *   편성을 줄이려면 그 조건만 지키면 된다.
   */
  ...
  const sim: BattleSim = {
    ...
    baseRange: () => 2,
    allyCap: () => 4,
    baseNextStats: () => ({ hpMax: 140, dmg: 13, range: 2.25, allyCap: 5 }),
  };
