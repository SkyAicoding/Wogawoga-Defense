 ...
 *
 * 왜 2.0이 아니라 2.1인가 (실측):
 *  타워 좌표는 셀 정수이고 경로는 셀 중심을 지난다. 정확히 2.0으로 잡으면 경로에서
 *  두 칸 떨어진 타워와의 거리가 **정확히 2.0** — 즉 정지 판정이 부등호의 칼날 위에
 *  선다. 적은 매 틱 speed×TICK_DT 만큼 이산적으로 뛰므로, 그 한 점을 밟는지는
 *  종별 speed와 스폰 위상에 달린 **우연**이 된다.
 *  실측(직선 경로 · 이격 2에 타워 4기(간격 4) · 각 종 8마리 · 정지 사격 비율 =
 *  정지 중 발사 / 전체 발사):
 *    2.0 → blade 33% · lancer **0%** · archer **0%** · hexer **0%** · warrior 50% (합 21%)
 *    2.1 → blade 43% · lancer 33% · archer 35% · hexer 27% · warrior 25% (합 34%)
 *  2.0에서는 다섯 중 셋이 **한 발도** 정지 사격을 못 했다. 우연히 멈추는 두 종도
 *  "멈추도록 설계돼서"가 아니라 speed가 그 점을 밟아서일 뿐이라, 속도를 조금만
 *  건드리면 조용히 0이 된다 — 데이터로 표현되지 않는 규칙은 규칙이 아니다.
 *  2.1이면 두 칸 타워 주위로 폭 **1.28타일**(= 2·√(2.1²−2.0²))의 정지 구간이 열려
 *  어떤 speed로도 최소 한 틱은 그 안에 들어간다. 나머지 66%는 사거리 진입부터
 *  정지 구간까지의 **걸으며 쏘기**로, 이 값이 100%가 아닌 것 자체가 규칙 4-b가
 *  살아 있다는 증거다.
 *  tests/sim/siege.test.ts '5종 전부 정지 사격을 한다'가 이 값을 잠근다.
 */
export const SIEGE_ENGAGE_RANGE = 2.1;
/**
