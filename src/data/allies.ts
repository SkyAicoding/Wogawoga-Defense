 ...
 *
 * ── 수치의 기준선 ──────────────────────────────────────────────────────────
 * 타워 T1 배치가 90~150골드다. clubber 55는 그 절반 — 되돌아오지 않는 20초짜리이므로
 * 영구 구조물보다 싸야 한다. 반대로 6명을 다 채우면 794골드(ALLY_COST_GROWTH 1.35)라
 * T2 업그레이드 두어 번 값이 나가서, "이번 웨이브를 넘길 것인가 다음을 준비할 것인가"가
 * 실제 선택이 된다.
 *
 * 수명 600틱(20초)은 웨이브 하나가 대체로 끝나는 길이다. 웨이브를 넘겨 살아남지
 * 않으므로 "쌓아 두는" 플레이가 원천적으로 불가능하다 — 이게 영구 유닛을 버린 이유다.
 *
 * ⚠ 최종 튜닝은 4단계 담당이다. 여기 값은 실측이 아니라 위 기준선에서 유도한 출발점이다.
 */
