...
export interface AllyState {
  ...
  targetId: number;
  /**
   * **회복 쿨다운 잔여 틱** (sim/heal.ts. 회복 능력이 없는 종은 언제나 0).
   *
   * 렌더가 읽는 값이라 공개 상태에 있다 — 사용자 지적:
   *   > "마법사가 hp 힐링 할때 애니메이션을 넣어줘. 지금은 가만 서 있어."
   * 회복은 `targetId` 를 안 쓴다(싸우는 것이 아니다). 그래서 뷰의 공격 채널이 0 으로
   * 굳어 마법사만 정지 자세로 얼어붙었다. `attackCdLeft` 와 **같은 규약**이라
   * (타격 순간 전체로 채워지고 매 틱 1씩 준다) 뷰가 같은 역산을 그대로 쓴다:
   *   지난 회복 이후 흐른 틱 = cooldown − healCdLeft · 다음 회복까지 = healCdLeft
   */
  healCdLeft: number;
  /**
...
export interface BattleStateView {
   ...
   * 그것도 새로고침마다 바뀐다. 그래서 "내 답 전부"를 보려면 전체 목록이 필요하다
   * (없는 답을 알려주는 것은 정보가 아니라 좌절이다 — 화면은 내 덱 안에서만 골라야 한다).
   * 시뮬레이션은 이 배열을 절대 수정하지 않는다 — hash()에도 들어가지 않는다.
   *
   * ⚠ **지금 이 필드를 읽는 화면은 없다** (2026-08-29). 읽던 곳은 없앤 웨이브 미리보기
   *   띠의 수요 막대뿐이었다. 계약 필드라 남긴다 — 값은 판을 만들 때 한 번 복사되고,
   *   손패 서열 표시 같은 것이 생기면 그날 바로 필요하다.
   */
