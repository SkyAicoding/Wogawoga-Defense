...
export class PlacementController {
  ...
  /**
   * **지금 고른 자원 칸 하나** (없으면 null) — 배지를 그 칸에만 켜기 위한 값.
   *
   * 사용자 지적: "1개의 자원을 선택하면 그 자원만 선택된 마름모 표지가 나와야 하는데,
   * 모든 자원들에 마름모 표지가 표시된다."
   *
   * 위 `showingResourceBadges` 의 두 조건이 **뜻이 다르다**는 것이 이 함수가 생긴 이유다:
   *  · 부족을 고르는 중  = "이제 **어디로** 보낼까" → 갈 수 있는 칸이 **전부** 켜져야 한다
   *  · 자원 칸을 고르는 중 = "**이** 칸이 뭐지"     → 그 칸 **하나만** 켜져야 한다
   * 종전에는 둘을 한 불리언으로 합쳐 놓아서 뒤쪽에서도 판 전체가 켜졌다.
   */
  focusedResourceCell(): Vec2 | null {
    return this.selectedAllyDef !== null ? null : this.selectedSceneryCell;
  }
