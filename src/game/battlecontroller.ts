...
/*
 * 궤도 드래그 감도 — **각도 폭과 같은 배율로 움직인다.**
 *
 * ⚠ 2026-08-29 사용자 요구로 둘 다 ×1.3 했다:
 *   > "오른쪽 마우스로 땅을 잡고 상하로 움직이거나 좌우로 움직였을때 각도를
 *   >  지금 보다 30% 정도 더 증가 시켜줘. 더 많이 상하로 각도를 바꿔 보고 싶어"
 *
 *   `camera.ts` 의 YAW_RANGE · PITCH_MIN/MAX 도 같은 ×1.3 이다. 둘을 **함께** 올려야
 *   제스처가 그대로 느껴진다 — 폭만 넓히면 같은 드래그로 못 다 돌아 "안 늘었다"로 읽히고,
 *   감도만 올리면 클램프에 더 빨리 부딪혀 "더 못 간다"가 그대로다.
 *   그래서 폭을 다 훑는 데 드는 드래그 거리는 전과 같다(요 ≈290px · 피치 ≈180px).
 */
/** 데스크톱 궤도 드래그 감도 — 요 허용폭 104°를 약 290px에 매핑 */
const ORBIT_YAW_PER_PX = 0.364 * DEG;
/** 피치 허용폭 32.5°를 약 140px에 매핑 (세로 화면에서 손가락이 짧다) */
const ORBIT_PITCH_PER_PX = 0.234 * DEG;
