...
export const ko: Record<string, string> = {
  ...
  /*
   * ── 문간 (11단계 2단계) ────────────────────────────────────────────────
   * 보스가 마을 문간에 서 있는 동안에만 화면에 나오는 낱말들이다.
   * 짧아야 하는 자리라 전부 6자 안쪽으로 잡았다 — 390px 폭에서 한 줄에
   * 아이콘·이름·HP바·초읽기가 같이 들어가야 한다.
   */
  // 집결 버튼 — 살아 있는 부족원 전원을 마을로 되부른다 (탭 1회)
  'battle.gate.rally': '집결',
  'battle.gate.rallyHint': '부족원 전원을 마을로 — 문간의 보스를 붙잡으면 물지 못해요',
  // 띠 제목. "문간"이 이 기능 전체의 이름이다 (src/sim/gate.ts)
  'battle.gate.title': '문간',
  // 초읽기. 소수 한 자리까지 — 1초 주기라 정수로 자르면 늘 "1초"만 보인다
  'battle.gate.next': '다음 한 입 {s}초',
  // 봉쇄 중 — 이 줄이 뜨는 것이 집결 버튼을 누른 보상이다 (gate.ts 규칙 2)
  'battle.gate.blocked': '붙잡는 중 — 안 물어요',
  // 스턴 — 봉쇄와 뜻이 달라 따로 적는다 (쿨다운까지 얼어붙는다)
  'battle.gate.stunned': '기절 — 안 물어요',
  // 한 입 크기 배지 (마을 HP 한 자리로 읽힌다)
  'battle.gate.bite': '−{n}',
  // 남은 한 입 수 = 지금 상태로 마을이 버티는 초 수와 같다 (한 입 = 1초)
  'battle.gate.bitesLeft': '{n}입 남음',
  // 둘 이상이 문간에 섰을 때 (미니보스 + 보스가 겹치는 후반 웨이브)
  'battle.gate.more': '×{n}',
