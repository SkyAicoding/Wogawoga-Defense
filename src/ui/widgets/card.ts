...
// ---------------------------------------------------------------------------
// 적 아이콘 16종 — **전부 코드로 그린다**
//
// ⚠⚠ **지금 이 표를 쓰는 화면은 없다** (2026-08-29). 유일한 소비자였던 웨이브 미리보기
//     띠를 사용자 요구로 없앴다(`screens/battlehud.ts` 의 제거 기록). 지우지 않고 둔 것은
//     도감·결과 화면이 언제든 쓸 수 있는 손그림이기 때문이고, 아래 규약 셋도 **그때 다시
//     지켜야 할 규약**으로 남긴다. 트리셰이킹이 못 걷어가므로 번들에는 11.8KB 원문이
//     그대로 실린다 — 정말 빼야 할 날이 오면 이 절(FOE_BODY ~ enemyIconSvg)을 통째로 지운다.
//
// 규약 셋 (되살릴 때 지킨다):
...
//     아이콘에서 본 것과 판 위에서 만나는 것이 같은 색이어야 두 화면이 이어진다.
