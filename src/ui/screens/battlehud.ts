...
export function createBattleHud(): Screen<GameFacade> {
  ...
  /*
   * ── 문간 띠 ── ⚠ **이 위젯은 이미 없다** (아래 제거 기록 참조) ────────────────
   *
   * 아래는 **설계 기록**이지 현재 화면 설명이 아니다. 현재형으로 쓰인 문장을 그대로
   * 읽지 마라 — 여기 적힌 두 줄·집결 버튼은 전부 사용자 요구로 사라졌다. 남겨 둔 이유는
   * 되살릴 때 같은 판단을 다시 하지 않기 위해서다(무엇을 왜 그 두 줄로 골랐는가).
   * 하단 배치의 **현재** 근거는 이 파일의 `const bottom = h('div', …)` 위 주석 하나뿐이다.
   *
   * 적이 마을 문 앞에 서 있는 **동안에만** 존재했다(src/sim/gate.ts). 두 줄이었다:
  ...
  return {
    enter(facade) {
       ...
       *   ⚠ **이 문단은 당시의 기록이라 그대로 둔다.** 미리보기 띠는 2026-08-29 에
       *     없앴지만(아래 제거 기록), 순서 개정이 고친 문제는 prep 요소가 **둘**이던
       *     그때의 크기다. 여기를 지금 상태로 고쳐 쓰면 근거가 절반으로 줄어 읽힌다.
       *
       *   지금 순서는 **뜨고 지는 것을 위로, 손이 가는 것을 아래로** 보낸다:
       *     웨이브시작                      ← 준비 단계에만 존재(미리보기 띠가 있던 자리다)
   ...
   *
   *   ⚠ **`card.ts` 의 적 아이콘 16종(`enemyIconSvg`)은 이제 아무도 안 쓴다.** 띠의 칩과
   *     상세가 유일한 소비자였다(이 파일의 임포트는 그전부터 이미 죽어 있었다 — 같이 걷어냈다).
   *     지우지 않고 둔 이유는 도감·결과 화면이 언제든 쓸 수 있는 손그림 자산이라서다.
   *     되살릴 때는 `widgets/card.ts` 의 `enemyIconSvg` 를 그대로 import 하면 된다.
   *
   *     ⚠ **다만 번들에서는 안 빠진다** (실측: 임포트를 지워도 index 청크 462.49KB 그대로,
   *       번들 안에 아이콘 path 문자열이 그대로 있다). `ENEMY_ICONS` 를 `SVG(...)` **호출**로
   *       짓기 때문에 롤업이 그 초기화를 순수하다고 증명하지 못해 통째로 남긴다. 값은
   *       11.8KB 원문(gzip 뒤 훨씬 작다)이라 지금은 두고 간다 — 정말 빼야 할 날이 오면
   *       테이블을 문자열 리터럴로 펴거나(`/* @__PURE__ *\/`) 그때 지우면 된다.
   *
